...
**Q1. World model이 AI에게 해주는 핵심 역할은 무엇일까요? 가장 잘 설명한 것을 골라보세요.**

1. 입력이 들어올 때마다 그에 맞는 정해진 규칙을 빠르게 찾아주는 것
2. 세계의 내부 표현을 만들어 현재 상태를 이해하고, 다음을 예측하고, 시나리오를 상상해 계획하게 해주는 것
3. 모든 데이터를 픽셀 단위로 저장해두었다가 그대로 재생하는 것
...
**해설**: World model은 사람이 커피잔을 들 때 물리학을 의식하지 않아도 되는 것처럼, AI가 현실의 내부 표현을 갖고 현재 상태 이해·미래 예측·시나리오 상상·시뮬레이션 학습을 하게 해줍니다. 단순히 입력에 반응하는 것을 넘어 앞을 내다보고 결과를 시뮬레이션할 수 있다는 점이 핵심이에요.

</details>

**Q2. 2018년 Ha와 Schmidhuber의 World Models 논문에서 가장 상징적인 아이디어는 "에이전트를 자신의 환각 속에서 훈련시킨다"는 것이었습니다. (O / X)**
...
**해설**: 이들은 환경을 생성 신경망으로 모델링하고 시공간 표현으로 압축한 뒤, 그 *학습된*(상상된) 환경 안에서 에이전트를 훈련시키고 정책을 실제 환경으로 전이했어요. 상상된 세계에서 훈련한 에이전트가 실제 작업을 해낸 것이 이후 모든 연구의 기초가 되었습니다.

</details>

**Q3. 같은 2024년 2월에 공개된 Sora(OpenAI)와 Genie(DeepMind)의 결정적인 차이는 무엇일까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: Genie의 세계는 키보드/마우스 입력으로 제어 가능한 "플레이 가능"한 인터랙티브 환경인 반면, Sora는 텍스트-비디오 생성기였습니다.

**해설**: Sora는 처음에 세계 모델로 마케팅되지 않은 텍스트-비디오 생성기였지만 기술 보고서에서 "세계 시뮬레이터로서의 비디오 생성"이라는 큰 그림을 드러냈어요. 반면 Genie는 명시적으로 세계 모델로 설계됐고, 가장 큰 차별점은 생성된 세계를 사용자가 직접 조작하며 플레이할 수 있다는 점이었습니다.

</details>

**Q4. "생성 진영"과 "예측 진영"의 철학적 분열에서, Yann LeCun의 Meta 팀(V-JEPA 계열)이 택한 접근은 어느 쪽일까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 예측 진영 — 픽셀을 생성하는 대신 잠재(latent) 공간에서 추상적 특징(임베딩)을 예측하는 쪽입니다.

**해설**: OpenAI·DeepMind가 "모든 것을 픽셀로 생성하라"는 생성 진영이라면, Meta는 "픽셀 생성은 계산 낭비이니 고수준 특징을 예측하자"는 예측 진영이에요. V-JEPA는 비생성적으로 잠재 공간에서 비디오의 누락 부분을 채워 학습하며, 픽셀 레벨 생성보다 효율적이라는 점을 내세웁니다.

</details>

**Q5. V-JEPA 2(2025)에서 사람들이 "미쳤다"고 평한 부분은, 웹 규모 비디오로 사전학습한 뒤 최소한의 로봇 데이터만으로 새로운 환경에서 제로샷 로봇 제어가 가능했다는 점입니다. (O / X)**
...
**해설**: "웹 규모 비디오 사전학습 + 최소한의 로봇 데이터 = 실행 가능한 세계 모델"이라는 조합이 핵심이었어요. 즉 YouTube 같은 웹 비디오로 훈련하고 로봇으로 일반화할 수 있다는 것인데, 본문 글쓴이도 응용 분야 중 로보틱스를 가장 기대하는 이유로 이 제로샷 일반화를 꼽았습니다.

</details>

**Q6. JEPA(결합 임베딩 예측 아키텍처)와 확산 모델(diffusion)의 장단점을 비교한 설명으로 옳은 것은?**

1. JEPA는 잠재 공간에서 임베딩을 예측해 효율적이지만 직접 해석이 어렵고, 확산 모델은 고품질 생성이 가능하지만 추론이 느리다
2. JEPA는 추론이 느리지만 해석이 쉽고, 확산 모델은 빠르지만 품질이 낮다
3. 둘 다 픽셀을 직접 생성하지 않으며 계산 비용도 동일하다
...
**해설**: JEPA는 잠재 공간에서 마스크된 영역의 임베딩을 예측해 효율적이고 추상적 특징을 학습하지만 직접 해석하기는 어렵습니다. 확산 모델(Genie, Epona 등)은 노이즈에서 구조화된 출력으로 노이즈를 제거하며 고품질·유연한 조건부 생성이 가능한 대신 추론이 느린 것이 단점이에요.

</details>

**Q7. 자율주행 회사의 안전팀에서 일한다고 상상해 보세요. 비 오는 밤 빙판길에서 보행자가 갑자기 튀어나오는 것처럼 "희귀하고 위험한" 상황을 차량에 안전하게 학습시키려면, World model이 어떻게 도움이 될까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 실제로 위험을 감수하지 않고도 희귀하거나 위험한 시나리오를 시뮬레이션으로 생성해 훈련·테스트할 수 있어, 안전 검증에 큰 도움이 됩니다.

**해설**: 자율주행차는 다음에 무슨 일이 일어날지 예측해야 하고, Epona 같은 모델은 분 단위 주행 시나리오를 고해상도로 생성할 수 있어요. 본문에서도 "훈련과 테스트를 위해 희귀하거나 위험한 시나리오를 시뮬레이션할 수 있는 것이 안전 검증에 엄청나다"고 자율주행을 명백한 승자로 꼽았습니다.

</details>

**Q8. 글쓴이가 정리한 "앞으로의 과제" 중 장기 일관성(long-term consistency) 문제에 대한 설명으로 맞는 것은?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 현재 대부분 모델은 수십 초(10~20초) 수준, 가장 앞선 Genie 3도 수분 정도의 일관성에 그치며, 시간 단위나 무한한 일관성이라는 목표에는 아직 도달하지 못했습니다.

**해설**: Genie 3는 분 단위, Sora는 최대 1분, 대부분 모델은 10~20초 정도의 일관성을 유지하는 수준이에요. 글쓴이는 "시간 단위 또는 무한한 일관성"을 목표로 제시하면서 아직 거기에 도달하지 못했다고 솔직하게 인정했고, 미래 예측 섹션에서도 더 긴 지평선을 주요 방향으로 꼽았습니다.

...
> 📊 **발표자료**: [claude-code-agent-teams.pptx](./claude-code-agent-teams.pptx)
...
**Q1. 서브에이전트와 에이전트 팀의 가장 결정적인 차이는 뭘까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 소통 방식이에요. 서브에이전트는 메인 에이전트에게만 결과를 보고하지만, 에이전트 팀의 팀원들은 서로 직접 메시지를 주고받으며 자기조율할 수 있어요.

**해설**: 둘 다 자체 컨텍스트 윈도우를 갖는 건 같은데요, 서브에이전트는 팀장이 일을 시키고 결과만 받아오는 구조라 팀원끼리 직접 얘기를 못 해요. 반면 에이전트 팀은 각 팀원이 완전히 독립적인 Claude Code 세션이라 서로 발견한 내용을 바로 전달하고, 공유 태스크 목록으로 스스로 작업을 조율하죠. 그래서 토론과 협업이 필요한 복잡한 작업엔 에이전트 팀이 맞아요.

</details>

**Q2. OX 퀴즈! 에이전트 팀에서는 팀 도중에 팀원을 리드로 승격시켜서 리더십을 넘길 수 있다. 맞을까요?**
...
**해설**: 팀을 생성한 세션이 팀의 존속 기간 동안 계속 리드를 맡아요. 팀원을 리드로 승격시키거나 리더십을 이전하는 건 현재 지원되지 않는 제약사항이에요. 이 외에도 리드는 한 번에 팀 하나만 관리할 수 있고, 팀원이 자신의 팀을 또 만드는 중첩 팀도 불가능하다는 점을 같이 기억해 두면 좋아요.

</details>

**Q3. 에이전트 팀의 네 가지 핵심 컴포넌트는 무엇일까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 팀 리드(Team Lead), 팀원(Teammates), 공유 태스크 목록(Shared Task List), 메일박스(Mailbox)예요.

**해설**: 팀 리드가 팀을 만들고 팀원을 소환해서 작업을 조율하고요, 팀원들은 각자 독립 세션으로 일해요. 공유 태스크 목록은 pending/in progress/completed 상태와 의존성을 관리하면서 파일 잠금으로 경합 조건을 막아주죠. 메일박스는 에이전트 간 비동기 메시지 시스템이라 리드가 일일이 폴링하지 않아도 메시지가 자동 전달돼요.

</details>

**Q4. 여러 에이전트가 같은 파일을 동시에 편집해야 할 때, 덮어쓰기 충돌 없이 작업하게 해주는 기능은 뭘까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: Git Worktree 격리예요. 서브에이전트라면 `isolation: worktree` 설정, CLI라면 `claude --worktree` 옵션을 쓰면 돼요.

**해설**: 각 에이전트가 자기만의 worktree에서 작업하니까 서로 간섭 없이 병렬로 돌 수 있어요. 예를 들어 에이전트 A와 B가 같은 파일을 서로 다른 방식으로 리팩토링한 뒤, 둘을 비교해서 더 나은 쪽을 고르거나 합칠 수도 있죠. 워크트리는 `.claude/worktrees/{name}/`에 생성돼요.

</details>

**Q5. 원인을 알 수 없는 버그를 조사할 때 "경쟁 가설" 패턴이 효과적인 이유는 뭘까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 여러 에이전트가 서로 다른 가설을 세우고 서로의 이론을 반박하게 하면, 한 가지 이론에 닻이 내려지는 걸 막을 수 있기 때문이에요.

**해설**: 에이전트 하나가 그럴듯한 이론을 하나 찾으면 거기에 고정돼서 다른 가능성을 잘 못 보게 되는데요. 과학 토론처럼 팀원들이 서로의 가설을 무너뜨리려고 시도하게 하면, 그 검증을 살아남은 이론이 진짜 원인일 가능성이 훨씬 높아져요. 혼자 답을 찾는 것보다 서로 검증하는 팀이 더 신뢰할 수 있는 결론에 도달하는 거죠.

</details>

**Q6. 응용 문제! "이전 단계의 결과가 나와야 다음 단계를 진행할 수 있는 마이그레이션 작업"이 있어요. 에이전트 팀을 쓰는 게 좋을까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 아니요, 이런 경우엔 단일 세션이나 서브에이전트가 더 나아요.

**해설**: 에이전트 팀의 가치는 병렬성에서 나오는데, 순차적 태스크는 병렬화할 게 없어서 토큰 비용과 조율 오버헤드만 떠안게 돼요. 본문에서도 순차적 태스크, 같은 파일 편집 작업, 의존성이 많은 작업, 빠른 일회성 작업에는 에이전트 팀을 쓰지 말라고 권하고 있어요. 병렬 코드 리뷰나 경쟁 가설 디버깅처럼 명확히 병렬성이 가치 있는 곳부터 시작하는 게 좋죠.

</details>

**Q7. 팀원을 소환할 때 소환 프롬프트에 태스크 특화 정보를 충분히 담아야 하는 이유는 뭘까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 팀원은 `CLAUDE.md`, MCP 서버, 스킬 같은 프로젝트 컨텍스트는 자동으로 로드하지만, 리드의 대화 히스토리는 넘어오지 않기 때문이에요.

**해설**: 팀원은 완전히 새로운 세션으로 시작하니까 리드가 사용자와 나눈 대화 내용을 전혀 몰라요. 그래서 검토할 모듈 경로, 집중할 포인트, 앱이 쓰는 기술 스택 같은 태스크 특화 정보를 소환 프롬프트에 직접 넣어줘야 해요. 모든 팀원이 공통으로 따라야 할 코딩 컨벤션은 `CLAUDE.md`에 넣어두면 자동으로 공유되고요.

</details>

**Q8. Claude Code CLI와 Claude Agent SDK는 각각 어떤 용도에 적합할까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: CLI는 터미널에서의 인터랙티브한 일상 개발에, Agent SDK는 CI/CD나 커스텀 앱 같은 프로그래밍 방식의 자동화에 적합해요.

**해설**: CLI는 사용자가 직접 개입하며 대화형으로 쓰는 도구라 자동화엔 제한이 있는데요, SDK는 코드로 완전히 제어할 수 있어서 프로덕션 자동화에 어울려요. SDK는 "맥락 수집 → 행동 수행 → 작업 검증 → 반복"이라는 에이전트 루프를 기본 구조로 제공하고, 많은 팀이 일상 개발엔 CLI를, 자동화 파이프라인엔 SDK를 함께 쓰죠.

...
**Q1. Chainlit은 Streamlit, Gradio와 어떤 점에서 가장 다른가요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: Chainlit은 처음부터 "대화형 AI 인터페이스" 전용으로 설계된 프레임워크예요. Streamlit은 데이터 앱/대시보드, Gradio는 ML 모델 데모가 주 용도죠.

**해설**: 그래서 Chainlit에는 채팅 버블, 응답 스트리밍, 인증, LangChain 네이티브 통합 같은 챗봇에 필요한 기능이 다 기본으로 들어있어요. 챗봇이나 AI 에이전트를 만든다면 Chainlit, 대시보드라면 Streamlit, 빠른 모델 데모라면 Gradio를 고르는 게 정석이에요.

</details>

**Q2. OX 퀴즈: 챗봇에서 대화 기록을 전역 변수(global variable)에 저장해도 멀티유저 환경에서 안전하다. (O/X)**
...
**해설**: 전역 변수를 쓰면 모든 사용자가 같은 데이터를 공유하게 돼서, 사용자 A의 대화가 사용자 B한테 섞이는 참사가 벌어지거든요. Chainlit은 이 문제를 해결하려고 사용자·세션별로 독립된 공간인 `cl.user_session`을 제공해요. 상태는 항상 `cl.user_session.set()` / `get()`으로 관리해야 안전하죠.

</details>

**Q3. 사용자가 메시지를 보낼 때마다 실행되는 함수를 만들려면 어떤 데코레이터를 써야 할까요? 채팅 세션이 시작될 때 초기화 코드를 실행하려면요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 메시지 처리는 `@cl.on_message`, 세션 시작 초기화는 `@cl.on_chat_start`예요.

**해설**: Chainlit의 핵심은 Flask처럼 간단한 데코레이터 기반 구조인데요. 이 둘 외에도 응답 생성 중단 시점의 `@cl.on_stop`, 세션 종료 시점의 `@cl.on_chat_end`, 이전 세션 재개 시점의 `@cl.on_chat_resume` 등이 있어요. 보통 `on_chat_start`에서 모델/체인을 만들어 세션에 넣어두고, `on_message`에서 꺼내 쓰는 패턴을 많이 써요.

</details>

**Q4. LLM 응답을 토큰 단위로 실시간 스트리밍하려면 Chainlit에서 어떤 흐름으로 코드를 짜야 하나요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 빈 `cl.Message`를 먼저 `send()`로 보내놓고, 토큰이 올 때마다 `stream_token()`으로 추가한 뒤, 마지막에 `update()`로 마무리하면 돼요.

**해설**: 빈 메시지를 먼저 띄워두고 토큰을 하나씩 채워 넣는 방식이라, 사용자 입장에서는 답변이 실시간으로 타이핑되듯 보이는 거예요. LangChain을 쓴다면 `cl.LangchainCallbackHandler()`를 콜백으로 넘겨서 체인의 중간 단계까지 자동으로 UI에 표시할 수 있죠.

</details>

**Q5. RAG 챗봇에서 "검색 → 분석 → 답변 생성" 과정을 사용자에게 접을 수 있는 UI로 보여주고 싶어요. Chainlit의 어떤 기능을 쓰면 될까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: `cl.Step`이에요. `async with cl.Step(name="검색") as step:` 형태로 감싸면 각 단계가 접을 수 있는 UI로 표시돼요.

**해설**: LLM이 어떤 과정을 거쳐 답을 냈는지 단계별로 보여주는 기능인데요. 각 단계의 `input`과 `output`도 기록할 수 있어서 디버깅할 때도 유용하고, 사용자 입장에선 AI가 뭘 하고 있는지 보이니까 신뢰도도 올라가거든요.

</details>

**Q6. 사내 직원만 쓸 수 있는 챗봇을 만들어야 해요. 구글 계정으로 로그인하되 회사 도메인 계정만 허용하고 싶다면, Chainlit에서 어떻게 구현하면 될까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: `@cl.oauth_callback` 데코레이터를 사용하면 돼요. 콜백 안에서 `raw_user_data`의 도메인 정보를 확인해서 회사 도메인이면 사용자를 반환하고, 아니면 `None`을 반환해 거부하는 식이죠.

**해설**: Chainlit은 패스워드 인증(`@cl.password_auth_callback`), OAuth(Google/GitHub/Azure AD 등), JWT 토큰까지 인증을 기본 내장하고 있어요. 콜백 함수가 `cl.User`를 반환하면 로그인 허용, `None`을 반환하면 거부라서 도메인 제한 같은 커스텀 로직을 넣기 쉽거든요.

</details>

**Q7. OX 퀴즈: LangGraph의 MemorySaver를 쓰면 멀티턴 대화 기록이 유지되는데, 이건 프로덕션에서도 그대로 쓰기에 충분하다. (O/X)**
...
**해설**: MemorySaver는 인메모리 체크포인터라서 서버가 재시작되면 대화 기록이 다 날아가요. 프로토타입에는 충분하지만, 프로덕션에서는 데이터베이스 같은 영속적인 체크포인터로 교체하는 게 권장되거든요. Chainlit 쪽에서도 마찬가지로 `@cl.data_layer` 데코레이터로 커스텀 데이터 레이어를 붙여서 대화를 DB에 저장할 수 있어요.

</details>

**Q8. Chainlit 앱을 로드밸런서 뒤에 여러 인스턴스로 배포하려고 해요. 꼭 챙겨야 할 설정은 뭘까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 로드밸런서에서 sticky session(세션 어피니티)을 활성화해야 해요. 다중 인스턴스라면 Redis 기반 세션 공유도 고려해야 하죠.

**해설**: Chainlit은 프론트엔드와 백엔드가 WebSocket으로 통신하는 구조라서, 같은 사용자의 연결이 매번 같은 인스턴스로 가야 세션이 유지되거든요. 그 외에도 프로덕션에서는 `-h` 플래그로 브라우저 자동 열기를 막고, 서브경로 배포 시 `--root-path`, CORS는 `config.toml`의 `allow_origins`로 설정하는 것도 기억해 두면 좋아요.

...
**Q1. TurboQuant가 압축하는 대상은 모델 가중치(weights)다. (O/X)**
...
**해설**: TurboQuant는 추론 중 메모리를 차지하는 KV 캐시(Key-Value Cache)를 압축하는 알고리즘이에요. 모델 가중치 자체를 줄이는 INT4/GPTQ와는 대상이 다르고, 두 방식은 서로 조합해서 함께 쓸 수도 있어요.

</details>

**Q2. TurboQuant의 "온라인(online)" 또는 "데이터 무관적(data-oblivious)"이라는 특징은 무엇을 의미할까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 데이터의 분포를 사전에 알거나 학습할 필요가 없다는 의미

**해설**: 데이터 분포를 미리 가정하지 않기 때문에 별도의 훈련(training)이나 미세조정(fine-tuning) 없이 즉시 적용할 수 있어요. 분포 가정이 필요 없다는 점이 KIVI나 Product Quantization 같은 데이터 의존적 방식과 구별되는 강점입니다.

</details>

**Q3. TurboQuant의 2단계 접근법에서 각 단계가 하는 역할을 짝지어 보세요.**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: Stage 1 = 무작위 회전 + Lloyd-Max 양자화로 대부분의 압축 수행(b-1 비트), Stage 2 = QJL 변환으로 양자화 오차를 1비트 보정해 편향 제거

**해설**: Stage 1에서 무작위 직교 회전을 적용하면 각 좌표가 알려진 Beta 분포를 따르게 되어 미리 계산한 최적 양자화기를 재사용할 수 있어요. Stage 2의 QJL은 남은 양자화 오차에만 1비트를 써서 내적 추정값을 무편향(unbiased)으로 만들어 정확도를 끌어올립니다.

</details>

**Q4. 무작위 직교 회전을 적용하면 각 좌표가 어떤 분포를 따르게 되며, 그게 왜 중요한가요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 각 좌표가 Beta((d-1)/2, (d-1)/2) 분포를 따르게 되고, 그 덕분에 미리 계산한 최적 스칼라 양자화기를 모든 벡터에 재사용할 수 있다.

**해설**: 회전 전에는 좌표 분포가 복잡하지만, 단위 구 위에서 무작위 회전을 거치면 각 좌표가 알려진 Beta 분포로 정리돼요. 분포가 미리 알려져 있으니 Lloyd-Max 코드북을 1회만 계산해 두면 차원과 비트폭만 맞으면 계속 재사용할 수 있습니다.

</details>

**Q5. TurboQuant와 KIVI를 비교할 때, TurboQuant가 갖는 핵심 장점 두 가지를 꼽아 보세요.**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: (1) 3비트에서도 완벽한 정확도를 유지(KIVI는 저비트에서 품질 손상), (2) 메모리 오버헤드를 완전히 제거하고 훈련이 불필요하며 이론적 보장이 있음

**해설**: KIVI는 구현이 더 단순하고 일부 프레임워크에 이미 통합돼 있다는 장점이 있어요. 반면 TurboQuant는 더 낮은 비트에서도 정확도를 지키고, 메모리 오버헤드가 0이며, 정보 이론 하한선에 대한 이론적 보장을 갖춰 실무 품질 면에서 앞섭니다.

</details>

**Q6. TurboQuant의 MSE 왜곡도가 정보 이론 하한선의 약 2.7배라는 것은 좋은 결과일까요, 나쁜 결과일까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 좋은 결과 (거의 최적에 가깝다는 의미)

**해설**: 정보 이론 하한선은 어떤 압축 방식도 이론적으로 넘어설 수 없는 한계예요. 그 하한선의 ~2.7배 수준이라는 건 데이터 분포를 전혀 모르는 상태에서도 이론적 최적에 매우 근접했다는 뜻이라, 성능이 우수하다는 근거가 됩니다.

</details>

**Q7. 장문맥 처리에서 시퀀스 도중에 회전 행렬을 새로 생성해 바꾸면 어떤 문제가 생길까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 앞뒤 토큰이 서로 다른 회전으로 양자화되어 표현이 불일치하게 되고 정확도가 깨진다.

**해설**: 회전 행렬은 양자화와 역양자화의 기준이 되므로, 한 시퀀스 안에서는 처음부터 끝까지 동일한 회전을 유지해야 해요. 베스트 프랙티스에서도 회전 행렬은 1회만 생성해 캐싱하고 전체 시퀀스에 같은 R을 재사용하라고 강조합니다.

</details>

**Q8. 현재 시점에서 TurboQuant를 도입할 때 가장 큰 실무적 한계는 무엇인가요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 공식 코드가 아직 미공개(Q2 2026 예정)이고 llama.cpp·vLLM 등 주요 프레임워크 통합이 부족하다는 점

**해설**: 논문과 커뮤니티 구현은 나와 있지만 Google의 공식 구현은 아직 공개 전이라, 생태계가 초기 단계예요. 또 알고리즘이 복잡해 최대 성능을 내려면 Triton 커널 같은 GPU 최적화가 필요하다는 점도 진입 장벽으로 꼽힙니다.

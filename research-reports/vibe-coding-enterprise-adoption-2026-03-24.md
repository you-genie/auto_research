...
**Q1. 바이브 코딩(Vibe Coding)을 한 문장으로 정의한다면, 어떤 개발 방식일까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 개발자가 자연어 프롬프트로 의도를 표현하고, AI가 생성한 코드를 **깊은 검토나 이해 없이 받아들이는** 개발 방식입니다.

**해설**: 본문은 바이브 코딩을 "직관적으로, 방법론적이지 않게" 작업하며 AI 출력을 깊이 검사하지 않고 신뢰하는 방식으로 정의합니다. 핵심은 단순히 AI를 쓰는 것이 아니라 "최소한의 인간 검토와 이해"로 코드를 수용한다는 점입니다. "돌아가면 맞는 거다(If it runs, it's correct)"라는 수용 편향이 그 태도를 잘 보여줍니다.

</details>

**Q2. 2026년 3월 아마존 장애가 이 리포트에서 "분수령(watershed moment)"으로 다뤄지는 이유는 무엇일까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: AI가 생성한 코드 변경이 6시간 쇼핑 장애와 630만 건의 주문 손실을 일으켰고, 이를 계기로 아마존이 업계 최초로 AI 코드에 대한 **시니어 사인오프 의무화**를 도입하며 거버넌스 시대를 열었기 때문입니다.

**해설**: 주니어·미드레벨 엔지니어가 AI 코드의 미묘한 결함을 잡지 못한 채 수용한 것이 근본 원인이었습니다. 이 사건은 "가드레일 없는 바이브 코딩은 위험하다"는 우려를 입증했고, **거버넌스 > 속도**라는 새로운 선례를 만들어 다른 기업들에 비슷한 통제를 압박했습니다.

</details>

**Q3. 본문은 "2025년은 AI 속도의 해, 2026년은 AI 품질의 해"라고 말합니다. (O / X)**
...
**해설**: 이 문장은 리포트의 핵심 발견(Key Finding)으로 맨 앞 요약에 등장하고 결론에서도 반복됩니다. 2025년에는 처리량·diff 양·PR 사이클 타임 같은 속도 지표가 칭송받았지만, 아마존 장애를 변곡점으로 2026년에는 결함률·보안 발견 같은 품질 지표로 무게중심이 옮겨가고 있다는 것이 전체 서사의 뼈대입니다.

</details>

**Q4. 아마존의 "시니어 사인오프" 정책과 마이크로소프트(GitHub Copilot Enterprise)의 거버넌스 접근은 어떻게 다를까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 아마존은 모든 AI 코드 변경에 **시니어의 인간 검토를 강제(blanket mandate)**하는 반응적·하향식 방식인 반면, 마이크로소프트는 조직별로 정책을 켜고 끄는 **계층형 거버넌스 + 자동화 도구(스캐닝, 리뷰 자동화)**를 통해 기업이 각자의 리스크 수준에 맞춰 자율적으로 채택하도록 합니다.

**해설**: 아마존은 위기 이후 인간 게이트를 의무화했지만 아직 자동 스캐닝으로 보완되지 않았습니다. 반대로 마이크로소프트는 "금지가 아닌 안전장치를 갖춘 활성화(enablement with safeguards)"를 지향하며, 순수 인간 게이트보다 더 나은 도구를 권합니다. 즉 강제 vs. 자율, 인간 게이트 vs. 도구 기반의 차이입니다.

</details>

**Q5. 본문이 인용한 연구에 따르면, AI가 생성한 코드의 상당 비율이 설계 결함이나 보안 취약점을 포함합니다. 이때 AI 모델이 보안에 취약한 코드를 만드는 근본 이유는 무엇일까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: AI 모델은 **"요구사항을 통과시키는 것(working)"에 최적화**되어 있을 뿐, 위협 모델을 고려한 보안 추론(security reasoning)을 할 동기가 없기 때문입니다. 게다가 학습 데이터 자체에 나쁜 패턴(취약한 코드)이 섞여 있습니다.

**해설**: 본문은 AI가 SQL 인젝션, 커맨드 인젝션, 약한 암호화 같은 취약점을 학습 데이터에서 그대로 따라 한다고 지적합니다. 핵심 통찰은 모델이 "돌아가는 코드"는 만들지만 "안전한 코드"를 만들 인센티브가 없다는 점입니다. 그래서 "린트를 통과했으니 괜찮다"는 식의 리뷰가 위험해집니다.

</details>

**Q6. "주니어 개발자 위기(Junior Developer Crisis)"의 역설(paradox)을 설명해 보세요.**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 조직은 시니어 인력을 채워 넣기 위해 주니어 개발자가 **필요한데**, 정작 AI가 주니어의 전통적 업무(코드 작성·디버깅)를 대체하면서 주니어가 성장하는 경로 자체를 **없애고 있다**는 점입니다. 그 결과 오늘의 주니어가 시니어가 되어야 할 5~10년 뒤에 "기술 사막(skill desert)"이 올 수 있습니다.

**해설**: 본문은 주니어가 코드를 생성할 수는 있지만 디버깅하거나 직접 작성한 코드가 아니라서 이해하지 못한다고 지적합니다. 기업들이 "더 적은 인력으로 더 많이(do more with less)"를 추구하며 주니어 파이프라인을 줄이면, 미래에 코드를 검증할 시니어가 부족해지는 구조적 위험이 발생합니다.

</details>

**Q7. 본문에 따르면 시니어 개발자의 역할은 아마존 사태 이후 어떻게 바뀌었나요? 시간 배분의 변화를 중심으로 답해 보세요.**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 시니어는 아키텍처 설계 위주에서 **AI 코드 감사자(Code Auditor)**로 무게중심이 옮겨갔습니다. 본문 기준 코드 리뷰 비중은 10% → 40%로 크게 늘고, 아키텍처/설계(40% → 30%)와 멘토링(25% → 15%)에 쓰는 시간은 줄어듭니다.

**해설**: 시니어는 이제 코드 감사자·프롬프트 가이드·가드레일 수호자·AI 로직 해설자 역할을 동시에 맡습니다. 다만 본문은 이로 인해 "코드 감사자 함정"(반복적이고 정신적으로 소모적인 일), 병목 효과, 고부가가치 작업 시간 감소 같은 부작용도 함께 지적합니다.

</details>

**Q8. 당신이 한 기업의 기술 리더라면, 이 리포트의 권고를 바탕으로 바이브 코딩 리스크를 줄이기 위해 가장 먼저 해야 할 일은 무엇일까요? (응용)**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 위기가 터지기를 기다리지 말고 **거버넌스를 일찍 도입**하되, 인간 리뷰에만 의존하지 말고 **자동 검증·보안 스캐닝 같은 도구에 투자**하며, 속도(처리량) 대신 결함률·보안 발견 같은 **품질 지표로 측정 기준을 전환**하는 것입니다.

**해설**: 본문의 "기업을 위한 핵심 권고"는 아마존에서 배워 선제적으로 거버넌스를 도입하고, 인간 게이트만으로는 부족하므로 계층형 방어(정적 분석 + 보안 검증 + 인간 리뷰 + 자동 테스트 + 런타임 모니터링)를 구축하라고 강조합니다. 동시에 생산성 이득과 품질 비용을 함께 측정해야 한다고 권합니다.

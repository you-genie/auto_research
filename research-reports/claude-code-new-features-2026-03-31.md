...
**Q1. Claude Code를 단순한 "인라인 코드 제안 도구"가 아니라고 설명하는 이유는 무엇인가요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 컨텍스트 수집 → 실행 → 검증으로 이어지는 완전한 에이전트 루프를 돌리는, 코드베이스 전체를 이해하고 조작하는 에이전트 개발 플랫폼이기 때문이에요.

**해설**: Claude Code는 단일 파일의 자동완성에 그치지 않고 파일 읽기·검색으로 컨텍스트를 모으고, 파일 편집·명령 실행으로 작업하고, 테스트를 돌려 결과를 검증하는 사이클을 자동으로 반복해요. 세션 간 학습을 저장하고 다중 에이전트 병렬 작업까지 조율하는 점에서 일반적인 코드 제안 도구와 구분됩니다.

</details>

**Q2. Skills(스킬)와 `/help`, `/compact` 같은 내장 명령어의 가장 큰 차이는 무엇인가요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 내장 명령어는 고정된 로직을 빠르게 실행하는 반면, 스킬은 프롬프트 기반이라 Claude가 상황을 판단하며 여러 도구를 조합해 실행해요.

**해설**: 스킬은 마크다운으로 작성한 "재사용 가능한 프롬프트 + 실행 규칙"이에요. 그래서 `/deploy`나 `/explain-code`처럼 호출하면 Claude가 지침을 따라가며 Read·Bash 등 도구를 알아서 조합합니다. 반면 내장 명령어는 정해진 동작만 수행하죠.

</details>

**Q3. 같은 이름의 스킬이 여러 레벨에 존재할 때 어느 쪽이 우선 적용될까요? 우선순위 순서를 떠올려 보세요.**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: Enterprise(org-wide) > Personal(~/.claude/) > Project(.claude/) > Plugin 순서이며, 상위 레벨이 하위를 override해요.

**해설**: 본문은 조직 전체에 배포되는 Enterprise 스킬이 가장 높은 우선순위를 가지고, 그다음 개인·프로젝트·플러그인 순이라고 설명해요. 같은 이름이면 더 상위 레벨의 정의가 덮어쓰기 때문에 조직 차원의 정책을 일관되게 강제할 수 있습니다.

</details>

**Q4. 스킬 frontmatter에서 `context: fork` 옵션은 어떤 효과가 있나요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 새로운 격리된 subagent 컨텍스트에서 스킬을 실행하고, 메인 세션에는 영향을 주지 않은 채 정리된 요약만 돌려줘요.

**해설**: `deep-research`나 `pr-summary` 예제처럼 `context: fork`를 쓰면 별도 컨텍스트가 생성됩니다. 덕분에 대량 파일 탐색 같은 무거운 작업을 해도 메인 세션의 컨텍스트가 오염되지 않고, 결과 요약만 깔끔하게 받아볼 수 있어요.

</details>

**Q5. "Plan mode(읽기 전용)는 가장 안전한 권한 모드다." — O일까요 X일까요?**
...
**해설**: Plan mode는 계획만 제시하고 실제 실행은 별도 승인을 받아야 하기 때문에, 본문에서도 "가장 안전(검토 필수)"으로 분류해요. Default, Auto-accept edits, Auto mode와 달리 읽기 전용으로 동작하므로 의도치 않은 변경 위험이 가장 낮습니다. 모드 전환은 Shift+Tab으로 합니다.

</details>

**Q6. Claude Opus 4.6의 컨텍스트 윈도우는 이전 세대 대비 어떻게 달라졌고, 입력/출력은 어떻게 구성되나요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 200K에서 1M 토큰으로 약 5배 늘었고, 입력 800K + 출력 200K로 총 1M 토큰을 구성해요.

**해설**: Sonnet과 Opus 4.5의 200K 대비 Opus 4.6 베타는 1M 토큰을 지원해 더 큰 프로젝트와 긴 세션을 다룰 수 있어요. 다만 본문은 1M도 장시간 작업에선 부족할 수 있다며 Skills·Subagents·CLAUDE.md·구조화된 메모리로 컨텍스트를 관리하라고 권합니다.

</details>

**Q7. `/batch` 스킬이 대규모 마이그레이션 같은 작업을 빠르게 끝낼 수 있는 핵심 메커니즘은 무엇인가요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 작업을 5~30개의 작은 unit으로 분해한 뒤 각 unit마다 git worktree를 만들어 여러 에이전트를 병렬 실행하고, 각각 PR을 생성해 결과를 집계해요.

**해설**: 본문은 `/batch`가 코드베이스를 분석해 작업을 잘게 나누고 worktree 기반으로 동시에 돌린다고 설명해요. 그래서 순차 실행 대비 약 30% 시간만으로 대규모 리팩터링을 끝낼 수 있다는 거죠. 격리된 worktree 덕에 작업끼리 충돌도 줄어듭니다.

</details>

**Q8. Claude Code Security(보안 스캔) 기능의 현재 한계로 본문이 언급한 것은 무엇인가요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 연속 스캔이 불가능해 한 번만 실행되고, 결과가 규정 준수(compliance) 인증이 아니며, 포괄적인 보안 프로그램을 대체할 수 없어요.

**해설**: SQL Injection·XSS·하드코딩된 자격증명 등을 AI 기반으로 감지해 주지만, 본문은 이를 연구 프리뷰 단계의 보조 도구로 명확히 선을 그어요. 따라서 정식 보안 감사나 컴플라이언스 절차를 완전히 대신할 수는 없다는 점을 기억해 두세요.

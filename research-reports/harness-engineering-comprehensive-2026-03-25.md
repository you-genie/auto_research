...
**Q1. 테스트 하네스를 구성하는 두 가지 핵심 요소로, 각각 "상위 레벨 컴포넌트를 시뮬레이션하는 것"과 "하위 레벨 컴포넌트를 대체하는 것"은 무엇일까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 드라이버(Driver)와 스텁(Stub)

**해설**: 드라이버는 테스트 중인 모듈을 호출하는 상위 레벨 컴포넌트를 시뮬레이션하고, 스텁은 테스트 중인 모듈이 호출하는 하위 레벨 컴포넌트를 대체합니다. 완전한 인프라를 사용할 수 없을 때 이 둘이 모의 인프라 역할을 해서 테스트를 가능하게 합니다.

</details>

**Q2. OpenAI의 2025년 정의에 따르면, 소프트웨어 하네스 엔지니어링을 구성하는 3가지 영역은 무엇일까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 컨텍스트 엔지니어링(Context Engineering), 아키텍처 제약(Architectural Constraints), 가비지 컬렉션(Garbage Collection)

**해설**: 컨텍스트 엔지니어링은 코드베이스의 지속적으로 개선되는 지식 기반과 동적 컨텍스트 접근을 다루고, 아키텍처 제약은 레이어 분리·커스텀 린터·명시적 경계로 구조를 강제하며, 가비지 컬렉션은 주기적으로 실행되는 에이전트가 문서 일관성을 검증하고 기술 부채를 지속적으로 상환합니다.

</details>

**Q3. (OX) Mock과 Stub은 같은 개념이며, 둘 다 호출 내역을 기록하고 검증하는 데 쓰인다.**
...
**해설**: Stub은 호출되면 미리 결정된 응답을 반환하는 단순한 구현으로, 상태 검증에 주로 쓰입니다. 반면 Mock은 호출 내역(횟수, 인자 등)을 기록하고 검증할 수 있는 객체로, 행동 검증(behavior verification)에 사용됩니다. 본문 예제에서도 Mock은 `assert_called_once_with`나 `call_count`로 상호작용을 검증했습니다.

</details>

**Q4. 테스트에서 "Fixture(픽스처)"가 담당하는 역할은 무엇일까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 테스트를 실행하기 위해 필요한 초기 상태를 설정(Setup)하고 정리(Teardown)하는 것

**해설**: 픽스처는 DB 초기화, 서비스 모킹, 샘플 데이터 준비 같은 사전 작업을 담당하며, 테스트가 끝나면 롤백·정리까지 책임집니다. pytest의 `@pytest.fixture`나 unittest의 `setUp`/`tearDown`이 대표적인 픽스처 메커니즘입니다.

</details>

**Q5. Harness Platform이 Jenkins와 차별화되는 대표 기능으로, 이전 테스트 결과를 분석해 변경사항과 관련된 테스트만 골라 실행함으로써 빌드를 빠르게 하는 기능은 무엇일까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: Test Intelligence (테스트 인텔리전스)

**해설**: Test Intelligence는 변경된 코드와 관련된 테스트만 선택적으로 실행해 빌드 시간을 크게 단축합니다. 본문 비교표에서 Harness는 이 기능을 내장하고 있는 반면 Jenkins는 별도 플러그인이 필요하고 GitLab CI·GitHub Actions에는 없는 것으로 정리되어 있습니다.

</details>

**Q6. Canary 배포에서, 처음부터 모든 트래픽을 새 버전으로 보내지 않고 소량(예: 5%)만 흘려보낸 뒤 헬스 체크와 메트릭을 검증하는 이유는 무엇일까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 문제가 생겼을 때 영향 범위를 최소화하고, 이상이 감지되면 자동 롤백할 수 있도록 하기 위해서

**해설**: 본문 CD 파이프라인 예제에서는 5% 트래픽으로 먼저 배포한 뒤 응답 시간·에러율 같은 메트릭을 검증하고(예: 200ms 초과 시 롤백), 정상이면 50% → 100%로 점진 확대합니다. 이렇게 하면 결함이 전체 사용자에게 퍼지기 전에 차단할 수 있습니다.

</details>

**Q7. 마이크로서비스 환경에서 Signadot 같은 도구가 제공하는 핵심 가치는, 변경된 서비스만 격리 배포하고 나머지는 프로덕션 버전을 그대로 쓰는 방식입니다. 이 접근이 주는 이점을 본문 근거로 설명해 보세요.**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 전체 스택을 복제하지 않고도 PR 단위로 격리된 E2E 테스트 환경을 빠르게 만들 수 있고, 프로덕션 데이터를 안전하게 유지할 수 있다

**해설**: 본문에서 Signadot은 변경된 서비스(예: payment-service:pr-123)만 샌드박스에 배포하고 user-service·notification-service 등 의존 서비스는 프로덕션 버전을 재사용합니다. 그 결과 변경사항만 격리 배포, 프로덕션 데이터 안전, E2E 테스트 고속화, PR 단위 테스트 환경이라는 이점을 얻습니다.

</details>

**Q8. (응용) 팀에서 단위 테스트는 매우 빠른데 전체 통합 테스트가 너무 느려 매번 PR마다 돌리기 부담스럽습니다. 본문에서 소개한 개념 중 어떤 것들을 조합해 피드백 루프를 짧게 유지할 수 있을까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 느린 테스트를 마커로 격리해 선택 실행(`@pytest.mark.slow` → `pytest -m "not slow"`), 병렬 실행(pytest-xdist `-n auto`), Test Intelligence로 변경 관련 테스트만 실행, 그리고 Google 테스트 피라미드처럼 빠른 단위 테스트 비중을 크게 두는 전략을 조합한다

**해설**: 본문 성능 최적화 절에서는 병렬 실행·세션 스코프 픽스처 재사용·Test Intelligence를, 확장성 절에서는 느린 테스트 격리를 다룹니다. 또한 Google 사례의 테스트 피라미드(단위 85%, 통합 10%, E2E 5%)는 빠른 테스트를 다수 두고 느리고 비싼 테스트는 최소화하는 원칙을 보여줍니다. 이들을 함께 적용하면 PR마다 빠른 피드백을 유지하면서 무거운 테스트는 push 시점 등으로 분리할 수 있습니다.

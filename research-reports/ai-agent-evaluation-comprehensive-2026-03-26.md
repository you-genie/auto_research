...
**Q1. `pass@k`와 `pass^k`는 비슷해 보이지만 측정하는 게 다릅니다. 둘의 차이는 무엇이고, 프로덕션 에이전트의 안정성을 볼 때 더 적합한 쪽은 어느 것일까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: `pass@k`는 k회 시도 중 최소 1회 이상 성공할 확률, `pass^k`는 k회 시도가 모두 성공할 확률입니다. 프로덕션 안정성에는 `pass^k`가 더 적합합니다.

**해설**: `pass@k`는 여러 솔루션을 뽑아 그중 하나만 맞으면 되는 코딩 에이전트 평가에 쓰이고, `pass^k`는 매번 일관되게 성공해야 하는지를 보기 때문에 안정성이 중요한 프로덕션 에이전트 평가에 적합합니다. 즉 "한 번이라도 되느냐"와 "항상 되느냐"의 차이입니다.

</details>

**Q2. 단일 에이전트 평가만으로 멀티 에이전트 시스템을 평가하기 어려운 이유를 두 가지 이상 설명해 보세요.**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: ① 독립성 가정 위반(한 에이전트의 출력이 다른 에이전트의 입력이 되어 연쇄 오류 발생), ② Emergent behavior(개별로는 멀쩡해도 조합 시 예측 불가능한 행동), ③ 동적 환경(에이전트 행동에 따라 환경이 변함), ④ 협력 비효율성(중복 통신·작업 중복·데드락) 중 두 가지 이상.

**해설**: 단일 에이전트 평가는 각 에이전트가 독립적·정적으로 작동한다고 가정하지만, 실제 멀티 에이전트는 출력이 서로의 입력이 되고 순서·타이밍·상태가 모두 얽혀 있습니다. 그래서 협력·통신·창발 행동을 따로 측정하는 새 프레임워크가 필요합니다.

</details>

**Q3. (OX) LLM-as-Judge(모델 기반 Grader)는 비결정적이고 비용이 높지만, 오픈엔디드한 작업의 뉘앙스를 잡아낼 수 있다는 장점이 있다.**
...
**해설**: 모델 기반 Grader는 유연하고 확장 가능하며 뉘앙스를 캡처하고 오픈엔디드 작업을 처리할 수 있다는 장점이 있습니다. 다만 비결정적이고 비용이 높아 인간 Grader와의 보정(calibration)이 필요하다는 단점이 있습니다. 반대로 코드 기반 Grader는 빠르고 객관적·재현 가능하지만 유효한 대안에 취약합니다.

</details>

**Q4. MAEBE 프레임워크가 평가하는 "Safety Emergence Risk" 중 Collusion, Deception, Reward Hacking은 각각 무엇을 의미하나요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: Collusion은 에이전트들이 협력해서 시스템 규칙을 우회하는 것, Deception은 멀티 에이전트의 조작·속이기 행동, Reward Hacking은 목표를 왜곡한 채 협력하는 것을 뜻합니다.

**해설**: MAEBE(2025)는 개별 에이전트의 의도된 행동에서 나타나지 않는 시스템 수준의 창발 행동을 다룹니다. 이런 위험은 specification 기반 평가, 행동 모니터링, 그리고 공격자 역할 에이전트를 투입하는 adversarial testing으로 측정합니다.

</details>

**Q5. Windows Agent Arena 결과에서 최고 모델은 19.5% 성공률, 인간은 74.5%였습니다. 같은 시기 연구용 벤치마크(예: SWE-Bench)에서는 80% 이상이 나오기도 했는데요. 이 큰 격차가 시사하는 바는 무엇일까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 정적·정형화된 연구용 벤치마크에서는 높은 점수가 나와도, 실제 OS 환경처럼 동적이고 멀티모달인 프로덕션성 작업에서는 아직 인간과 큰 격차가 있다는 것을 보여줍니다.

**해설**: 본문은 "연구용: 80%+ 성공률, 프로덕션: 19.5%(아직 멀음)"으로 이 간극을 짚습니다. 그래서 2026년 트렌드는 정적 벤치마크 신뢰에서 벗어나 실제 환경·동적 변화·리소스 제약을 반영한 평가로 이동하고 있습니다.

</details>

**Q6. Anthropic은 평가를 Capability Evals와 Regression Evals로 나눕니다. 둘은 각각 어떤 Pass rate 양상을 보이며 왜 그런가요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: Capability Evals는 아직 못 하는 새로운 능력을 측정하므로 낮은 Pass rate(예: <50%)로 시작하고, Regression Evals는 기존에 되던 기능이 유지되는지 확인하므로 높은 Pass rate(예: >95%)를 유지해야 합니다.

**해설**: Eval-driven development에서는 새 기능에 대해 일부러 0% pass인 Eval을 먼저 만들고(아직 못 하는 게 정상), 새 모델이 나오면 그 베팅이 성공했는지 확인합니다. 반대로 Regression Eval이 떨어지면 회귀(퇴행)가 일어난 신호입니다.

</details>

**Q7. 본문이 제시한 통합 평가 아키텍처의 최종 점수 공식은 `Final Score = 0.4×TC + 0.3×CQ + 0.2×R + 0.1×EI`입니다. 가장 큰 가중치를 받는 축은 무엇이며, 이 가중 구조가 말하려는 바는 무엇일까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 가장 큰 가중치(40%)는 Task Completion(작업 완료)이고, 그 뒤로 Collaboration Quality(30%), Robustness(20%), Economic Impact(10%) 순입니다.

**해설**: 여전히 "작업을 제대로 끝냈는가"가 핵심이지만, 협력 품질·견고성·경제적 영향까지 함께 반영해 단일 지표(정확도)만 보지 말라는 메시지를 담고 있습니다. 숫자 자체보다 "여러 축을 가중 합산해 종합 판단한다"는 구조가 핵심입니다.

</details>

**Q8. 어떤 에이전트가 검색 도구를 잘 쓰는지 평가하려고 합니다. "검색이 필요한 질문" 100개만 모아 테스트셋을 만들었습니다. 본문 모범 사례에 비춰 이 설계의 문제는 무엇이고 어떻게 고쳐야 할까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 테스트셋이 한쪽으로 편향되어 있습니다. 검색해야 하는 경우와 검색하면 안 되는 경우를 섞어(예: 50:50) Balanced Test Set으로 구성해야 합니다.

**해설**: 검색이 필요한 케이스만 넣으면 에이전트가 "무조건 검색하는" 쪽으로 최적화되어도 만점을 받습니다. 본문은 검색이 불필요한 상황을 절반쯤 포함해, 도구를 "써야 할 때 쓰고 안 써야 할 때 안 쓰는지"까지 평가하라고 권합니다.

</details>

**Q9. 본문은 "에이전트가 Eval을 실패했다고 곧바로 평가 실패로 결론짓지 말라"는 Anthropic의 교훈을 강조합니다. 왜 그럴까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: Eval 자체에 모호성·오류·잘못된 Grader가 있을 수 있으므로, 에이전트 실패를 단정하기 전에 Eval(과 Reference Solution, Grader)이 제대로 작동하는지 먼저 검증해야 하기 때문입니다.

**해설**: 모범 사례에서 모든 Task에는 작동하는 Reference Solution이 있어야 하고, 이는 Eval이 실제로 평가 가능한지·Grader가 올바른지·모호성이 없는지를 보증합니다. 자동화에는 한계가 있어 인간 리뷰와 샘플링을 병행하라는 권고와도 맞닿아 있습니다.

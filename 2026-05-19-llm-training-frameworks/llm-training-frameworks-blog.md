...
> 📊 **발표자료**: [llm-training-frameworks-presentation.pptx](./llm-training-frameworks-presentation.pptx)
...
**Q1. LLM 학습 프레임워크를 5개 계층으로 나눌 때, 계층이 낮을수록 어떤 특징이 있을까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 제어권이 많고 복잡도가 높아요. 반대로 계층이 높을수록 편의성은 좋지만 커스터마이징이 제한되죠.

**해설**: Layer 1(저수준 분산 엔진, 예: Megatron-Core, DeepSpeed)은 GPU 간 통신과 병렬화 전략까지 직접 다룰 수 있는 대신 다루기 어렵고, Layer 2 이상(NeMo 같은 End-to-End 플랫폼)은 검증된 레시피로 빠르게 시작할 수 있는 대신 세밀한 제어가 어려워요. 이 트레이드오프가 프레임워크 선택의 핵심 축이에요.

</details>

**Q2. OX 퀴즈! "DeepSpeed의 ZeRO는 단계가 올라갈수록(ZeRO-1 → ZeRO-3) 메모리 절감 효과가 커지지만, 통신 오버헤드도 함께 커진다." 맞을까요?**
...
**해설**: ZeRO-1은 옵티마이저 상태만 분할해서 통신 부담이 낮고, ZeRO-2는 그래디언트까지, ZeRO-3는 파라미터까지 분할하면서 메모리 절감 폭이 커지는 대신 통신 오버헤드도 높아져요. ZeRO-Infinity는 CPU/NVMe 오프로드로 거의 무제한 메모리를 쓰지만 통신 비용이 매우 높죠. 메모리와 통신의 트레이드오프가 핵심이에요.

</details>

**Q3. FSDP2가 기존 FSDP(v1)와 가장 크게 달라진 점은 뭘까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 샤딩 단위가 모듈 단위에서 파라미터(텐서) 단위로 바뀌었고, DTensor 기반으로 재설계됐어요.

**해설**: FSDP2는 DTensor를 기반으로 텐서 수준에서 샤딩을 관리해서 메모리 레이아웃을 세밀하게 제어할 수 있어요. 덕분에 Tensor Parallel과의 조합이 쉬워지고 torch.compile도 완전 지원하게 됐죠. 이 조합이 중소 규모 학습의 사실상 기본 스택이 되어가고 있어요.

</details>

**Q4. Megatron-Core와 NeMo의 관계를 설명해 보세요. 둘은 어떤 계층에 속하고, 어떻게 연결되어 있을까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: Megatron-Core는 Layer 1(저수준 분산 학습 엔진), NeMo는 Layer 2(End-to-End 학습 플랫폼)이고, NeMo가 Megatron-Core를 백엔드로 사용해요.

**해설**: Megatron-Core는 TP/PP/EP 같은 병렬화 전략을 담당하는 핵심 엔진이고, NeMo는 그 위에서 데이터 로딩, 학습 레시피, 체크포인팅까지 통합해 더 높은 추상화를 제공해요. NVIDIA GPU를 쓰는 중규모 기업이라면 NeMo부터 고려하는 게 일반적이죠.

</details>

**Q5. 2025년 RL 포스트 트레이닝 분야에서 가장 빠르게 표준화되고 있는 프레임워크와 그 개발사는 어디일까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: ByteDance Seed 팀이 개발한 veRL(HybridFlow)이에요.

**해설**: veRL은 EuroSys 2025에 HybridFlow 논문으로 발표됐고, hybrid-controller 모델로 학습과 추론(vLLM/SGLang)을 효율적으로 분리·통합한 게 핵심 설계예요. o1, R1 같은 추론 모델의 부상으로 RL 인프라가 핵심 경쟁력이 되면서 UC Berkeley, Tsinghua 등 다양한 기관이 채택하며 빠르게 표준이 되어가고 있죠.

</details>

**Q6. 응용 시나리오! 단일 GPU 한 장으로 LoRA 파인튜닝을 해보려는 개인 개발자에게 본문 기준으로 가장 적합한 도구는 뭘까요?**
...
**해설**: Unsloth는 단일 GPU에서 타 도구 대비 2~3배 빠른 학습이 가능하고 메모리도 훨씬 적게 써서, 단일/소수 GPU 환경의 메모리 효율로는 최강이에요. 다만 오픈소스 버전은 멀티 GPU 지원이 제한적이라, GPU 여러 장을 쓴다면 Axolotl이나 TRL + PEFT 쪽이 더 맞아요.

</details>

**Q7. OX 퀴즈! "프론티어 랩이 아닌 대부분의 기업도 경쟁력을 위해 자체 학습 프레임워크를 처음부터 개발하는 것이 일반적이다." 맞을까요?**
...
**해설**: 풀스택 자체 개발은 NVIDIA, Meta, Google, DeepSeek(HAI-LLM)처럼 극단적 규모를 다루는 프론티어 랩만의 선택이에요. 나머지 기업들은 NeMo, veRL, TorchTitan 같은 오픈소스 생태계를 잘 활용하는 게 현실적인 정답이죠. 본문 결론에서도 프레임워크는 수단일 뿐, 팀의 GPU 수와 역량에 맞게 고르라고 강조했어요.

</details>

**Q8. RL 학습 파이프라인에 vLLM이나 SGLang 같은 '추론 엔진'이 들어오게 된 이유는 뭘까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: RL 학습에서 rollout generation(응답 생성)이 병목이 됐기 때문이에요.

**해설**: RL 포스트 트레이닝은 모델이 직접 답변을 생성(rollout)하고 그걸 평가해서 학습하는 구조인데, 이 생성 단계가 전체 속도를 좌우하는 병목이에요. 그래서 veRL, OpenRLHF, slime 모두 고속 추론 엔진인 vLLM/SGLang을 rollout 엔진으로 파이프라인 안에 통합했죠. 학습과 추론의 경계가 허물어지는 2025~2026년 대표 트렌드예요.

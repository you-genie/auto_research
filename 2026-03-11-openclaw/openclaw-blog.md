...
> 📊 **발표자료**: [openclaw-presentation.pptx](./openclaw-presentation.pptx)
...
**Q1. OpenClaw가 ChatGPT 같은 일반 챗봇과 결정적으로 다른 점은 뭘까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: '눈과 손'을 가지고 있다는 점이에요. 웹 브라우징, 파일 읽기/쓰기, 쉘 명령어 실행을 자율적으로 수행하고, 사용자가 묻기 전에 AI가 먼저 일하고 보고하는 구조예요.

**해설**: 일반 챗봇은 대화 창에서 질문해야 답을 주는 수동적 도구인데요, OpenClaw는 내 기기 위에서 24시간 돌아가면서 스케줄에 맞춰 알아서 작업을 처리해요. "사용자가 시켜야 반응"이 아니라 "AI가 먼저 일하고 보고"한다는 게 핵심 차이죠.

</details>

**Q2. OX 퀴즈! OpenClaw를 쓰려면 전용 앱을 새로 설치해서 그 앱 안에서만 대화해야 한다. (O/X)**
...
**해설**: OpenClaw의 가장 큰 특징이 바로 멀티채널 메신저 연동이에요. Telegram, WhatsApp, Slack, Discord, iMessage 등 20개 이상의 플랫폼을 지원해서, 쓰던 메신저 그대로 AI 비서를 부릴 수 있죠. 별도 앱이 필요 없다는 점이 진입장벽을 확 낮춘 비결이에요.

</details>

**Q3. 프로젝트 이름이 Clawdbot → Moltbot → OpenClaw로 두 번 바뀌었는데요, 각각 어떤 이유였을까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 첫 번째 변경(Clawdbot → Moltbot)은 Anthropic의 상표권 문제 제기 때문이고, 두 번째 변경(Moltbot → OpenClaw)은 오픈소스 정체성과 발톱(claw) 모티프를 살리려는 자발적 결정이었어요.

**해설**: "Clawd"가 Anthropic의 Claude와 발음이 너무 비슷해서 상표권 분쟁이 생겼고, 로브스터 탈피(molt)에서 따온 Moltbot으로 바꿨는데요. 이름을 바꾸자마자 사이버범죄자들이 유사 도메인으로 악성코드를 뿌리는 사태까지 겹쳤어요. 결국 최종적으로 OpenClaw라는 이름에 정착했죠.

</details>

**Q4. OpenClaw의 브라우저 자동화는 스크린샷 기반이 아니라 다른 방식을 쓰는데요, 어떤 방식이고 왜 더 좋을까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 접근성 트리(Accessibility Tree) 방식이에요. 화면 이미지를 분석하는 것보다 더 정확하고 빠르죠.

**해설**: 스크린샷을 찍어서 이미지를 해석하는 방식은 느리고 오류가 생기기 쉬운데요, 접근성 트리는 웹 페이지의 구조 정보를 직접 읽기 때문에 버튼이나 입력란을 정확하게 찾을 수 있어요. 최신 자동화 에이전트들이 가는 추세와도 일치하는 방향이에요.

</details>

**Q5. ClawHub에서 스킬을 설치할 때 주의해야 하는 이유는 뭘까요? 그리고 어떻게 대비하면 될까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 보안 감사 결과 ClawHub 스킬 중 341개(약 12%)가 악성 코드를 담고 있었기 때문이에요. 설치 전에 GitHub 저장소, 별 수, 최근 업데이트 일자, 리뷰를 확인하고 검증되지 않은 스킬은 설치하지 않는 게 대비책이에요.

**해설**: Koi Security가 ClawHub의 전체 스킬을 감사했더니 'ClawHavoc'이라는 조직적 캠페인에서 나온 악성 스킬이 무더기로 발견됐어요. 주로 크립토 트레이딩 봇이나 생산성 도구로 위장하고 있었죠. 누구나 스킬을 올릴 수 있는 오픈 마켓플레이스라서 사용자 스스로 검증하는 습관이 필수예요.

</details>

**Q6. OpenClaw 아키텍처에서 Gateway와 Pi Agent Runtime은 각각 어떤 역할을 맡고 있나요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: Gateway는 로컬 WebSocket 제어평면으로 모든 채널의 메시지를 수신·라우팅하고 세션 관리와 권한 제어를 담당해요. Pi Agent Runtime은 실제 추론을 담당하는 에이전트 런타임으로 LLM과 연결되는 부분이에요.

**해설**: 메신저 앱에서 들어온 메시지는 먼저 Gateway를 거쳐서 Pi Agent Runtime으로 전달되는 구조인데요, 추론이 필요한 일은 Runtime이 LLM과 함께 처리하고 스킬(MCP 서버)을 호출해요. 역할이 깔끔하게 분리되어 있어서 채널을 추가하거나 스킬을 핫-리로드하기 쉬운 구조죠.

</details>

**Q7. 응용 문제! 친구가 "OpenClaw 설치했는데 외부에서도 접속할 수 있게 포트를 열어두려고 해"라고 하면, 뭐라고 조언해줘야 할까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 절대 그러지 말라고 해야 해요. 기본 설정 `dmPolicy="pairing"`을 유지하고, `openclaw doctor` 명령으로 보안 상태를 정기 점검하라고 조언해주면 돼요.

**해설**: 외부에 포트를 열어둔 채 기본 설정을 그대로 쓰는 노출 인스턴스가 4만 개 이상 발견됐는데요, 이런 상태면 외부에서 누구나 내 AI 에이전트에 접근할 수 있어요. 게다가 ClawJacked 취약점처럼 악성 웹사이트가 로컬 인스턴스에 무단 연결해서 임의 명령을 실행하는 공격도 실제로 있었으니, 보안 설정은 꼭 챙겨야 해요.

</details>

**Q8. nanobot은 OpenClaw와 비교해서 어떤 프로젝트이고, 어떤 사람에게 추천할 만할까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 홍콩대 HKUDS 연구팀이 만든 초경량 버전으로, 약 4,000줄의 Python 코드로 OpenClaw의 핵심 기능(MCP 통합, 멀티채널)을 구현했어요. OpenClaw의 내부 동작을 공부하고 싶은 사람에게 추천할 만해요.

**해설**: OpenClaw 본체는 코드가 방대해서 소스를 읽으며 공부하기가 버거운데요, nanobot은 핵심만 남긴 간결한 구조라 학습용으로 최적이에요. 이 외에도 ZeroClaw, PicoClaw, IronClaw 같은 파생 프로젝트들이 우후죽순 생겨나면서 'Claw Craziness'라고 불릴 정도로 생태계가 커지고 있죠.

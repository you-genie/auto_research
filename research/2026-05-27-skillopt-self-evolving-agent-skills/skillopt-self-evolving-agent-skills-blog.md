...
> 📊 **발표자료**: [skillopt-self-evolving-agent-skills-presentation.pptx](./skillopt-self-evolving-agent-skills-presentation.pptx)
...
**Q1. SkillOpt가 최적화하는 대상은 모델 가중치가 아니라 무엇일까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: frozen 에이전트에게 context로 주어지는 자연어 스킬 문서(마크다운 지시서)예요.

**해설**: SkillOpt는 모델 가중치는 전혀 건드리지 않고, 스킬을 "동결된 에이전트의 외부 상태(external state)"로 보고 text-space에서 훈련해요. 딥러닝 optimizer가 가중치를 업데이트하듯, 스킬 문서를 edit 단위로 업데이트하는 거죠.

</details>

**Q2. OX 문제: SkillOpt의 Optimizer 모델은 스킬 훈련이 끝난 뒤 배포 시점에도 계속 호출된다. (O/X)**
...
**해설**: Optimizer는 훈련 시에만 동작하고, 배포할 때는 최적화된 `best_skill.md` 파일 하나만 있으면 돼요. 그래서 추론 시 추가 LLM 호출이 0회이고, 배포 오버헤드가 없다는 게 SkillOpt의 큰 실용적 장점이죠.

</details>

**Q3. SkillOpt의 "Bounded Text Updates"는 딥러닝의 어떤 개념에 대응하고, 왜 필요한 걸까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: learning rate에 대응해요. 한 스텝에 적용 가능한 edit 횟수를 예산 L_t로 제한해서, 스킬 문서가 한 번에 너무 많이 바뀌어 망가지는 걸 막아요.

**해설**: learning rate가 너무 크면 학습이 발산하듯, 스킬도 제어 없이 자유롭게 수정하면 성능이 떨어진다는 걸 실험으로 보여줬어요. constant, linear decay, cosine decay 같은 스케줄까지 딥러닝과 똑같이 적용했죠.

</details>

**Q4. Validation Gate에서 후보 스킬이 채택되는 조건은 뭘까요? 동점이면 어떻게 될까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: held-out validation split에서 점수가 strictly improving, 즉 엄격하게 더 좋아질 때만 채택돼요. 동점은 거절이에요.

**해설**: 이 엄격한 게이트 덕분에 제안된 edit의 95% 이상이 거절되고, 최종 스킬에 들어가는 accepted edit은 벤치마크별로 1~4개 수준이에요. "나빠지면 어떻게 할지"에 대한 제어가 약했던 기존 자기 수정 방식과의 핵심 차이점이죠.

</details>

**Q5. Rejected-Edit Buffer와 Meta Skill은 둘 다 일종의 "메모리"인데요, 둘의 역할은 어떻게 다를까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: Rejected-Edit Buffer는 거절된 edit들을 모아서 다음 스텝의 optimizer에게 "이건 이미 해봤는데 안 됐어"라는 음성 피드백을 주는 스텝 단위 메모리이고, Meta Skill은 어떤 edit 전략이 효과적이었는지를 요약해 미래 에포크의 optimizer 프롬프트에 prepend하는 에포크 단위 메모리예요.

**해설**: 둘 다 optimizer 측을 돕는 장치인데, 버퍼는 같은 실수의 반복을 막고 meta skill은 장기적인 최적화 전략을 학습시켜요. 참고로 meta skill은 배포 artifact에 포함되지 않고 순전히 훈련용이에요.

</details>

**Q6. Ablation 실험에서 제거했을 때 가장 큰 성능 하락을 일으킨 구성 요소는 무엇이었을까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: Epoch-Wise Slow Update와 Meta Skill이에요. SpreadsheetBench에서 이 둘을 제거하면 가장 큰 폭의 하락이 나타났어요.

**해설**: 에포크 경계에서 이전 vs 현재 스킬을 비교해 종단적 가이던스를 protected section에 쓰는 메커니즘인데, 이게 빠지면 22.5포인트나 떨어졌죠. step-level edit이 건드릴 수 없는 보호 영역과 장기 패턴 분석이 그만큼 중요하다는 뜻이에요.

</details>

**Q7. 응용 시나리오: Codex harness에서 훈련한 SpreadsheetBench 스킬을 Claude Code harness에 그대로 가져다 쓰면 어떤 일이 벌어졌나요? 이 결과가 시사하는 바는 뭘까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 큰 폭의 성능 향상이 일어났고, 심지어 Claude Code에서 직접 훈련한 in-domain 참조 결과까지 초과했어요.

**해설**: 이건 학습된 스킬 문서가 특정 harness의 구현 세부사항이 아니라, 더 추상적인 절차적 지식을 인코딩하고 있다는 걸 시사해요. 한 번 잘 훈련한 스킬을 다른 환경에서 재사용할 수 있다는 점에서 실용적 함의가 크죠.

</details>

**Q8. SkillOpt에도 한계가 있는데요, 주관적 평가가 중요한 도메인(예: 글쓰기 스타일)에 바로 적용하기 어려운 이유는 뭘까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: Validation Gate가 작동하려면 점수화된(scored) 자동 검증 피드백이 필요한데, 주관적 평가 도메인에는 그런 명확한 보상 신호가 없기 때문이에요.

**해설**: SkillOpt의 모든 채택/거절 결정이 validation 점수 비교에 기반하기 때문에, 자동 검증자가 없으면 루프 자체가 돌아가지 않아요. 그래서 저자들도 "보상 없는 validation gate"를 향후 연구 방향으로 제안하고 있죠.

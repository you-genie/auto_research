...
> 📊 **발표자료**: [browser-use-agents-presentation.pptx](./browser-use-agents-presentation.pptx)
...
**Q1. AI 에이전트가 컴퓨터 화면을 인식하는 세 가지 대표적인 방식은 무엇인가요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 스크린샷 기반, DOM/HTML 파싱, 접근성 트리(Accessibility Tree) 활용 방식이에요.

**해설**: 스크린샷 방식은 어떤 화면이든 동작하지만 느리고 좌표 오류가 날 수 있고, DOM 파싱은 정확하고 빠르지만 동적 콘텐츠에 취약하죠. 접근성 트리는 빠르고 신뢰성이 높지만 접근성을 지원하지 않는 사이트에서는 실패할 수 있어요. 그래서 OpenAI CUA처럼 뛰어난 에이전트들은 여러 방식을 조합해서 쓴답니다.

</details>

**Q2. OX 퀴즈: 전통적인 RPA 툴은 웹사이트 UI가 조금만 바뀌어도 스크립트가 깨질 수 있다.**
...
**해설**: UiPath 같은 전통 RPA는 버튼 위치나 좌표를 코드에 딱 박아두는 방식이라, UI가 바뀌면 전면 재작업이 필요해요. 반면 AI 기반 에이전트는 화면을 보고 "이해"해서 스스로 요소를 찾아내기 때문에 자가 적응(Self-healing)이 가능하죠.

</details>

**Q3. Playwright MCP가 기존 스크린샷 방식 대신 채택한 인식 방식은 무엇이고, 그 장점은 뭔가요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 접근성 트리(Accessibility Tree) 방식이에요. 구조화된 텍스트 기반 표현이라 빠르고 결정론적인 제어가 가능하죠.

**해설**: 스크린 리더가 사용하는 구조화된 요소 정보를 그대로 활용하기 때문에, 비전 모델이나 스크린샷이 아예 필요 없어요. 덕분에 E2E 테스트나 폼 자동화처럼 정밀함이 필요한 작업에 잘 맞고, 주요 에이전트들도 점점 이 방향으로 이동하는 추세예요.

</details>

**Q4. Browser Use와 Anthropic Computer Use의 가장 큰 차이는 무엇인가요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 제어 범위가 달라요. Browser Use는 브라우저 안에서만 동작하는 오픈소스 라이브러리인 반면, Computer Use는 OS 전체 GUI를 제어할 수 있는 Anthropic의 API 기능이에요.

**해설**: Browser Use는 Python + Playwright 기반으로 스크린샷과 HTML을 함께 활용하는 MIT 라이선스 오픈소스예요. Computer Use는 화면 스크린샷을 좌표 그리드로 변환해 가상 키보드·마우스로 데스크톱 전체를 조작하는 방식이고, 오픈소스가 아니라는 점도 차이점이죠.

</details>

**Q5. WebVoyager 벤치마크에서는 90%대 성공률이 나오는데, 왜 본문에서는 "인간 감독이 여전히 필수"라고 강조했을까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 실제 비즈니스 워크플로우를 측정하는 CUB 벤치마크에서는 최고 성능이 10%대에 그치기 때문이에요. 벤치마크와 실전 사이의 갭이 아직 크다는 거죠.

**해설**: WebVoyager 같은 벤치마크는 비교적 정형화된 웹 태스크를 측정하지만, CUB는 실제 106개 비즈니스 워크플로우를 다뤄요. 복잡한 현실 업무에서는 에이전트가 아직 신뢰하기 어려운 수준이라, Human-in-the-loop 감독이 꼭 필요하다는 결론으로 이어지는 거예요.

</details>

**Q6. Skyvern 2.0의 "Route Memorization" 기능은 어떤 방식으로 속도와 적응력을 동시에 잡았나요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: AI가 처음 경로를 파악하면 이를 고속 Playwright 스크립트로 "컴파일"해 두고, 사이트가 변경되면 AI가 다시 깨어나 경로를 "치유(heal)"하는 방식이에요.

**해설**: 평소에는 미리 컴파일된 스크립트로 빠르게 실행하다가, UI가 바뀌어 스크립트가 깨지면 그때만 LLM이 개입해요. 매번 LLM 추론을 돌리는 비용을 아끼면서도 전통 RPA의 약점인 "UI 변경에 취약함"을 극복한 똑똑한 절충안이죠.

</details>

**Q7. 응용 시나리오: 회사에서 웹 서비스의 E2E 테스트를 자동화하려고 해요. 본문 기준으로 어떤 도구들이 가장 적합할까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: Playwright MCP와 Stagehand가 추천 도구예요.

**해설**: 둘 다 개발자 친화적인 도구인데요. Playwright MCP는 접근성 트리 기반의 결정론적 제어로 E2E 테스트에 최적화되어 있고, GitHub Copilot 코딩 에이전트와도 통합돼요. Stagehand는 TypeScript 기반에 요소·액션 자동 캐싱으로 LLM 비용 없이 재사용이 가능해서 반복 테스트에 잘 맞죠.

</details>

**Q8. OX 퀴즈: 2025~2026년 트렌드 중 하나는 에이전트가 클라우드 VM에서 작동하고 사용자는 결과만 받는 모델의 확산이다.**
...
**해설**: Project Mariner나 Manus처럼 클라우드 서버에서 에이전트가 돌아가는 모델이 확산되고 있어요. 로컬 리소스를 아끼면서 보안도 강화할 수 있거든요. 특히 Manus는 사용자가 연결을 끊어도 클라우드에서 작업을 계속 이어가는 게 차별점이었죠.

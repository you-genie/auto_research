...
> 📊 **발표자료**: [kormo-presentation.pptx](./kormo-presentation.pptx)
...
**Q1. KORMo 연구진이 정면 돌파하려 했던 핵심 딜레마는 무엇이었나요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 저자원 언어(한국어)는 고품질 실제 데이터가 부족한데, 그렇다고 합성 데이터를 대규모로 쓰자니 model collapse가 우려된다는 딜레마예요.

**해설**: 영어는 수천억 토큰짜리 고품질 코퍼스가 넘쳐나지만 한국어는 그렇지 못하죠. 합성 데이터로 부족분을 채우면 분포가 좁아지면서 모델이 망가질 수 있다는 게 기존 통념이었는데, KORMo는 한국어 코퍼스의 68%를 합성으로 채워도 안전하게 학습할 수 있는지를 직접 실험으로 검증했어요.

</details>

**Q2. OX 문제: 합성 데이터를 대규모로 사전학습에 쓰면 무조건 model collapse가 일어난다.**
...
**해설**: KORMo의 핵심 발견(RQ1)은 "조건부 YES"예요. 단일 합성기로만 만든 데이터는 collapse를 일으키지만, 서로 다른 스타일과 편향을 가진 여러 합성기의 출력을 섞으면 안정적으로 수렴하죠. 즉 합성 데이터 비율 자체보다 "어떻게, 누가 만드느냐"가 더 중요하다는 거예요.

</details>

**Q3. KORMo가 model collapse를 피하기 위해 합성 데이터 설계에서 의도적으로 확보한 두 가지 다양성은 무엇인가요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 시드 다양성과 합성 모델 다양성이에요.

**해설**: 시드 소스로는 FineWeb2, UltraFineWeb, Nemotron-CC-HQ, Cosmopedia 등 여러 출처를 쓰고, 합성 모델도 Qwen3-30B-A3B, GPT-OSS-120B, Qwen3-235B 세 종류를 섞었어요. 각 모델이 서로 다른 스타일과 편향을 갖고 있어서, 이들이 만든 데이터를 섞으면 단일 모델 분포로 수렴하지 않게 되는 거죠.

</details>

**Q4. 영어와 한국어의 최적 합성 데이터 비율은 어떻게 달랐고, 그 이유는 뭐라고 해석됐나요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 영어는 합성 비율을 85%까지 높여도 괜찮았지만, 한국어는 15%가 스위트 스팟이었어요.

**해설**: 한국어 데이터가 부족하니 합성을 더 많이 쓰면 될 것 같지만, 실험 결과는 반대였죠. 합성 모델들이 영어를 더 잘 알아서 영어 합성 데이터가 더 다양하고 품질이 고른 반면, 한국어 합성 데이터는 아직 다양성이 부족하기 때문으로 해석돼요(RQ2).

</details>

**Q5. Ko-Reasoning 데이터셋이 단순 번역 데이터와 다른 점은 무엇인가요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 영어 reasoning chain(사고 과정)과 한국어 답변을 짝지은 이중 언어 쌍이라는 점이에요.

**해설**: Qwen3-235B로 영어 추론 과정을 생성하고 답변은 한국어로 작성하는 방식인데요, 이 덕분에 모델이 사전학습 단계부터 한국어로 추론하는 능력을 익히게 돼요. 추가로 2단계 번역 필터(Algorithm 1)로 오역이나 품질 저하 데이터를 99% 걸러내서 순도를 확보했죠.

</details>

**Q6. 중복 제거(dedup) 실험에서 데이터의 70%가 줄었는데도 어떤 결과가 나왔고, 이게 시사하는 바는 뭔가요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 데이터가 70% 줄었는데도 성능이 오히려 향상됐어요. 데이터 품질이 양보다 훨씬 중요하다는 걸 실증적으로 보여주는 결과죠.

**해설**: KORMo는 가장 엄격한 Old-both 전략으로 dedup을 적용했는데요, 중복되고 질 낮은 데이터를 대량으로 걸러내자 학습 효율이 좋아졌어요. "데이터를 많이 모으는 것"보다 "좋은 데이터만 남기는 것"이 사전학습에서 더 중요하다는 교훈이에요.

</details>

**Q7. 응용 시나리오: 여러분이 베트남어 같은 또 다른 저자원 언어로 LLM을 처음부터 만든다면, KORMo의 발견에서 어떤 가이드라인을 가져올 수 있을까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: (1) 합성 데이터를 쓰되 최소 2~3가지 서로 다른 스타일의 합성기를 믹스하고, (2) 해당 언어의 합성 비율을 무작정 높이지 말고 최적 비율을 실험으로 찾고, (3) 대상 언어 토큰을 전체의 5% 이상 확보하는 거예요.

**해설**: KORMo는 단일 합성기는 collapse, 다중 합성기는 stable이라는 레시피를 제시했고, 언어·토크나이저마다 최적 합성 비율이 다르다는 것도 보였어요(한국어는 15%). 또 LLaMA-2의 0.06% 수준으로는 부족하고 5% 이상의 대상 언어 비중이 필요하다는 가이드라인도 함께 제시했죠.

</details>

**Q8. KORMo가 결과 수치와 별개로 연구 생태계에 기여한 점, 즉 Fully Open Model(FOM)로서의 의의는 무엇인가요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 데이터, 코드, 학습 레시피에 학습 로그까지 전부 공개해서 다른 연구자들이 학습 과정 전체를 재현하고 들여다볼 수 있게 했다는 점이에요.

**해설**: 베이스 모델의 한국어 벤치마크 절대값은 아직 낮지만, "저자원 언어에서 합성 68% 사전학습이 가능하다"는 걸 대규모로 증명하고 그 과정을 통째로 공개한 게 핵심 기여예요. 저자원 언어 LLM 연구의 레퍼런스 포인트가 될 수 있는 작업이죠.

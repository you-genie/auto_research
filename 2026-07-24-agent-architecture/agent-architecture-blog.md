...
**이 리포트가 답하려는 질문** — "에이전트 = LLM + 툴"이라는 요약은 2023년까지만 유효했다. 2026년의 프로덕션 에이전트는 **하네스·컨텍스트 엔지니어링·MCP·스킬·메모리·플래닝·멀티에이전트·평가/가드레일**이 겹겹이 쌓인 시스템이다. 이 글은 **상상할 수 있는 가장 복잡한 에이전트 하나**를 상정하고, 완성에 실제로 필요한 모든 구성 요소를 레이어별로 해부한다. 각 레이어마다 **개념 → 1차 자료 → 상용에서 실제로 쓰는 것**을 붙였다.
</div>

<div class="toc" markdown="1">
<span class="toc-title">목차 — 8개 레이어</span>
1. [하네스 / 에이전트 루프](#l1)
2. [컨텍스트 엔지니어링](#l2)
3. [툴 / MCP / 스킬](#l3)
4. [메모리 & 검색(RAG)](#l4)
...
```mermaid
flowchart TB
  subgraph P["🧭 관통 원칙 · 컨텍스트 경제(smallest high-signal token set)"]
    direction TB
    L1["L1 · 하네스 / 에이전트 루프"]
    L2["L2 · 컨텍스트 엔지니어링"]
    L3["L3 · 툴 / MCP / 스킬"]
    L4["L4 · 메모리 &amp; RAG"]
    L5["L5 · 플래닝 / 추론 / 자기수정"]
    L6["L6 · 멀티에이전트 오케스트레이션"]
    L7["L7 · 평가·관측·가드레일·신뢰성"]
    L8["L8 · 상용 프로덕션 스택"]
    L1 --- L2 --- L3 --- L4 --- L5 --- L6 --- L7 --- L8
  end
  classDef layer fill:#eef4fb,stroke:#4a7fb5,stroke-width:1px,color:#1F4E79;
  class L1,L2,L3,L4,L5,L6,L7,L8 layer;
```

---

## 0. 리포트를 관통하는 프레임: "가장 복잡한 에이전트"의 8-레이어 참조 아키텍처 {#sec0}
...
### 3.6 Clarification / Elicitation — 에이전트가 사용자에게 되묻기

에이전트가 "정보가 부족하다"고 판단해 **사용자에게 질문하고 답을 기다리는** 컴포넌트다. 자율 루프의 종료 조건(§1.4)이 "툴 호출 없는 평문"이라면, clarification은 그 반대편 — **루프를 의도적으로 멈추고(pause) 인간 입력을 대기**하는 지점이다. 실무에서 자주 뭉뚱그려지지만, 세 가지 **별개** 메커니즘으로 나뉜다.

| 종류 | 발동 조건 | 성격 |
|---|---|---|
| **Clarification(명확화)** | 진행하려는데 요구가 **모호·불충분** | "무엇을 원하세요?" — 열린 질문 |
| **Approval/Confirmation(승인)** | **위험·비가역** 행동 직전 | "실행해도 돼요?" — 게이팅 |
| **Interrupt(중단)** | 사용자가 끼어들어 방향 전환 | "그만/이거 말고" |

**툴 레이어 — Claude Code `AskUserQuestion`(네이티브).** Claude Code는 clarification을 **전용 빌트인 툴로 승격**했다. 모호하면 Claude가 *질문 + 선택지(옵션)를 스스로 생성*(코드베이스/컨텍스트를 분석해 "말이 되는 옵션"을 자동 제시)하고, **모달로 렌더링, 답이 올 때까지 루프를 블록**한다. 이는 §3.1 툴 설계 원칙의 사례다 — Anthropic은 질문하기를 툴로 승격시켜 "모달로 렌더링하고, 선택지를 제시하고, 답이 올 때까지 에이전트 루프를 블록"하게 만든다. 핵심은 **clarification(정보 필요)과 approval(권한 게이팅)을 명시적으로 분리**한다는 점(파일 삭제 승인은 permission 흐름, 정보 부족은 `AskUserQuestion`).

**Codex는 네이티브로 두지 않는다 — 설계 철학이 반대.** Codex는 **자율·비동기(sandbox-per-task, fire-and-forget)** 지향이라, 되묻기가 **큐를 블록**시키므로 의도적으로 약하게 설계된다(같은 이유로 Google Jules도 "라이브 채팅이 아니라 큐잉"). 그래서 Codex는 네이티브 clarification 툴 대신 **가정을 세우고 명시**하거나 계획/AGENTS.md 단계에서 선행 질문하는 쪽으로 흐르고, 부족분은 **커뮤니티의 instruction-only 스킬**("모호하면 현재 이해를 복기 → 1~5개 객관식 질문 → 정규화 JSON으로 재개")로 흉내 낸다. Codex의 **approval mode**(suggest/auto-edit/full-auto)는 위 표의 *2번(승인)* 이지 clarification이 아니다. 즉 **Claude Code = 빌트인 네이티브 / Codex = 자율지향, 스킬로 보완**.

**프로토콜 레이어 — 표준으로도 1급 개념.**
- **MCP: Elicitation** — `elicitation/create`(`ElicitationRequest`). MCP *서버가 실행 도중 클라이언트를 통해 사용자 입력을 요청*(예: "구독 취소" → 여러 개면 "Pro와 Enterprise 중?"). 2026 로드맵에서 stateless 재설계(`InputRequiredResult`)로 다중 왕복도 지속연결 없이.
- **A2A: `input-required` Task 상태** — Task 상태기계가 `submitted→working→**input-required**→…`. 에이전트가 작업을 멈추고 "입력 필요" 상태로 신호(승인 대기는 `auth-required`로 별도).
- **Anthropic Managed Agents** — `always_ask` 정책 → `user.tool_confirmation`, 세션이 `stop_reason: requires_action`로 idle(주로 승인이지만 동일한 pause-and-wait 기질).

**모델 행동 레이어 — "언제 물을지"도 조절 대상.** 툴이 있어도 *언제 그것을 쓰느냐*는 학습·프롬프트로 튜닝된다. Opus 4.8은 이전 세대보다 **더 신중해 사소한 것도 자주 물어보는** 경향이라, 실무에선 "사소한 선택(변수명·기본값)은 진행, 범위 변경·비가역만 물어라"로 ask-rate를 낮춘다. 반대로 **완전 자율 모드**에서는 되묻기가 곧 작업 중단이므로 "묻지 말고 합리적으로 가정하고 진행하라"로 억제한다 — Codex가 clarification을 약하게 두는 논리와 정확히 같다.

**요약**: clarification은 실재하는 별도 컴포넌트다. **Claude Code는 `AskUserQuestion`(네이티브 툴)**, MCP/A2A는 **Elicitation / `input-required`(프로토콜 프리미티브)** 로 구현하며, **Codex는 자율·비동기 설계 때문에 네이티브로 두지 않고 가정+명시/스킬로 대체**한다는 게 대비점이다.

### 3.7 Steering — 큐잉 · 주입 · 인터럽트 (입력 채널)

§3.6이 "에이전트가 사용자에게" 되묻는 방향이라면, steering은 반대 — **사용자가 진행 중인 에이전트에 끼어드는** 방향이다. 하네스가 입력 채널을 에이전트 루프에서 **비동기 큐로 분리**했기에 가능하다(§1.4). 두 연산으로 나뉜다: **Queue/Inject(소프트)** — 멈추지 않고 추가(다음 턴 큐 or 현재 턴 주입) — 와 **Interrupt(하드)** — 진행 중 생성/툴을 안전 지점에서 중단(= §3.6 3분법의 세 번째).

- **Claude Code — `h2A` 비동기 이중버퍼 큐**: Promise 기반 async iteration(`Symbol.asyncIterator`), backpressure 제어, 초당 1만+ 메시지, **pause/resume**. UI 입력과 루프 사이에 앉아 실시간 조향(steering)을 가능케 한다. 키: **Enter(작업 중)=큐잉**("queued message"), **Ctrl+C=인터럽트 후 큐 소비**, **Esc=진행분 폐기**.
- **Codex CLI — turn 기반 큐 + `turn/interrupt`**: **Tab=다음 턴 큐**, **Enter=현재 턴 주입(inject)**, **Esc=현재 태스크 인터럽트**(app-server의 `turn/interrupt` RPC; bounded queue, 포화 시 JSON-RPC `-32001`).
- **API 레벨**: Anthropic **Managed Agents**는 서버측 이벤트 큐(순서 처리 + `processed_at`) + **`user.interrupt`가 큐를 새치기**해 세션을 `idle`로 강제. OpenAI **Realtime API**는 **`response.cancel`** + **`conversation.item.truncate`**(barge-in/말 끊기).

<div class="callout callout-warn" markdown="1">
**Enter의 의미가 정반대다.** Claude Code에서 Enter는 *큐잉*(작업은 계속됨)이지만, Codex에서 Enter는 *현재 턴 주입*이다. "Enter를 눌렀는데 왜 안 멈추지?"가 두 도구를 오갈 때 흔한 함정. Claude Code에서 즉시 끊으려면 **Ctrl+C**(또는 폐기는 **Esc**).
</div>
...
## 9. 종합: "가장 복잡한 에이전트"의 전체 조립도 {#sec9}

이제 8개 레이어를 하나로 조립하면, 우리가 상정한 가장 복잡한 에이전트는 이렇게 생긴다.

```mermaid
flowchart TB
  U["🎯 사용자 목표<br/>이 리포지토리에 기능 X 구현 → PR"] --> H
  subgraph H["하네스 · 단일 메인 루프 (gather → act → verify → repeat) · L1/L2"]
    direction TB
    G["① gather · JIT 검색<br/>스킬 L1 메타(~100토큰) · agentic RAG · memory tool 되읽기<br/>L3 · L4"]
    A["② act · 툴/MCP 호출<br/>Tool Search(defer_loading) · code mode 압축 · todo.md 계획<br/>병렬 서브에이전트 팬아웃 · L3 · L5 · L6"]
    V["③ verify · 검증<br/>테스트 게이트 · evaluator-optimizer · 에러를 컨텍스트에 남김<br/>L5 · L7"]
    C["④ 한계 근처<br/>compaction + 진행상황 파일 기록(교대근무 핸드오프) · L2"]
    G --> A --> V --> C --> G
  end
  H --> OBS["관측 OTel span tree · 가드레일 lethal trifecta 방어<br/>durable execution · 코스트(라우팅/캐시/버짓) · L7"]
  OBS --> S["격리 샌드박스 VM 실행 → PR 생성 → CodeQL / 리뷰 게이트 · L8"]
  classDef goal fill:#1F4E79,stroke:#12324f,color:#fff;
  classDef step fill:#eef4fb,stroke:#4a7fb5,color:#1F4E79;
  classDef ops fill:#fff7ed,stroke:#e0a458,color:#8a4b0a;
  classDef out fill:#e9f7ef,stroke:#3fa66a,color:#1c6b3f;
  class U goal; class G,A,V,C step; class OBS ops; class S out;
```

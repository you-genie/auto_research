...
> 📊 **발표자료**: [latent-reasoning-presentation.pptx](./latent-reasoning-presentation.pptx)
...
>
> **2026-05-18 업데이트 3**: 2026년 1-5월 사이에 폭발적으로 쏟아진 신규 연구 12편을 종합한 **"2026년 신규 연구 종합"** 섹션 신설. 이론적 한계 분석(Capabilities and Limits, CoT Compression), 새 학습 방법(Abstract CoT, RLTT, PLaT, SIM-CoT, Supervised Thinking States, LT-Tuning, Inference-Time Rethinking), Looped 아키텍처(Ouro), 도메인 확장(VLA, MLLM), 분야 입장 논문(LLM Reasoning Is Latent), 벤치마크(LongCoT)까지. 이 5개월 동안 분야가 단숨에 성숙기에 진입했어요.
...
9. [2026년 신규 연구 종합 — 5개월에 쏟아진 12편](#9-2026년-신규-연구-종합--5개월에-쏟아진-12편)
10. [5가지 흐름 종합 — 분야 전체 지형도](#10-5가지-흐름-종합--분야-전체-지형도)
11. [미해결 과제와 향후 전망](#11-미해결-과제와-향후-전망)
12. [참고 문헌](#참고-문헌)
...
## 9. 2026년 신규 연구 종합 — 5개월에 쏟아진 12편

지금까지 정리한 6개 핵심 논문(Survey, Coconut, HRM, Soft Thinking, Quiet-STaR, RecursiveMAS) + 해석 연구 5편 외에도, **2026년 1-5월에만 12편 이상의 신규 latent reasoning 논문**이 쏟아졌어요. 이 5개월이 분야의 변곡점이었어요. 단순히 "또 한 편 나왔다" 수준이 아니라, 이론적 기반·새 학습 방법·도메인 확장·평가 표준화가 동시에 일어나면서 분야가 단숨에 성숙기에 진입한 시기죠.

5개 카테고리로 묶어볼게요.

### 9.1 이론적 기반 — "왜 latent가 어려운가"가 명료해짐

지금까지 latent reasoning의 한계는 empirical하게만 알려져 있었어요. Coconut의 GSM8K 한계, curriculum 의존성 같은 거. 2026년 1-2월에 나온 두 논문이 이걸 **수학적으로 증명**했어요.

**[Capabilities and Fundamental Limits of Latent CoT](https://arxiv.org/abs/2602.01148)** (arXiv 2602.01148, 2026-02)

핵심 정리:
- **Exploration-Execution trade-off 정리**: 단일 certainty level이 두 능력을 동시에 최적화할 수 없음을 증명
- **Symbolic Index** 도입: "decisional commitment"의 정량 측도. 이 값에 따라 exploration vs execution 우열이 결정됨
- **Curriculum learning이 이론적으로 essential**: 직접 학습은 distributional mismatch로 실패한다는 걸 증명

이 논문이 Coconut의 perplexing 패턴을 깔끔하게 설명해줘요 — 왜 ProsQA에서는 97%(exploration 우위)인데 GSM8K에서는 34.1%(execution 약점)인지. "low certainty가 search에 도움되지만 computational task에서는 noise를 amplify"한다는 거예요.

**[Chain Of Thought Compression: A Theoretical Analysis](https://arxiv.org/abs/2601.21576)** (arXiv 2601.21576, 2026-01)

핵심 결과:
- **High-order logical dependency가 exponentially decay**: implicit CoT가 intermediate step을 skip하면 학습 신호가 지수적으로 감소함을 증명
- **Order-r Interaction**: 왜 implicit compression이 어려운지에 대한 information-theoretic 설명
- **ALiCoT (Aligned Implicit CoT)** 제안: latent token 분포를 intermediate reasoning state와 align. **54.4x speedup**을 explicit CoT 성능 유지하면서 달성

이 두 편이 분야의 이론적 토대를 만들었어요. 이제 "왜 안 되는가"가 단순한 inductive bias 부족이 아니라 정량적으로 어디서 정보가 새는지 알 수 있게 됐어요.

### 9.2 새 학습 방법 — Coconut의 한계 7가지를 하나씩 깨다

7편의 신규 학습 방법론이 2026년 1-4월에 발표됐어요. 각각 Coconut의 다른 한계를 노립니다.

**[Abstract Chain-of-Thought (ACoT)](https://arxiv.org/abs/2604.22709)** (arXiv 2604.22709, 2026-04, "Thinking Without Words")
- **차별점**: continuous embedding이 아니라 **reserved discrete vocabulary**(별도 abstract token 집합)를 사용. Discrete의 해석 가능성 + latent의 효율을 절충
- **방법**: (i) verbal CoT masking + SFT, (ii) abstract token으로 self-distillation 교대
- **결과**: 자연어 CoT 대비 **11.6x 토큰 감소**, 수학/instruction-following/multi-hop reasoning에서 비슷한 성능
- **발견**: abstract vocabulary에 **emergent power law 분포** (자연어와 유사)
- **Coconut과 차이**: 사람이 abstract token을 볼 수 있어요 (해석성 부분 회복)

**[RLTT: Rewarding Latent Thought Trajectories](https://arxiv.org/abs/2602.10520)** (arXiv 2602.10520, 2026-02)
- **차별점**: looped LM의 **internal step 전체에 reward 분배**. 표준 GRPO는 final output에만 reward.
- **Base**: Ouro-2.6B-Thinking
- **결과**: GRPO 대비 **MATH-500 +14.4%, AIME24 +16.6%, BeyondAIME +10.0%**. 수학만 학습해도 비수학 task에 effective transfer
- **시사점**: latent reasoning은 process reward와 결합할 때 진정한 잠재력이 나옴

**[PLaT: Latent CoT as Planning](https://arxiv.org/abs/2601.21358)** (arXiv 2601.21358, 2026-01)
- **차별점**: reasoning을 **deterministic latent planning trajectory**로 모델링. Decoder가 별도로 text grounding 담당.
- **장점**: **동적 termination** — 모델이 스스로 reasoning을 언제 끝낼지 결정. Coconut의 fixed c 값에서 벗어남
- **트레이드오프**: greedy accuracy는 낮지만 inference-time search 확장성이 훨씬 좋음 ("broader solution space")

**[SIM-CoT: Supervised Implicit CoT](https://github.com/InternLM/SIM-CoT)** (arXiv 2509.20317, ICLR 2026; Fudan + Shanghai AI Lab)
- **차별점**: **Auxiliary decoder**로 step-level supervision. 추론 시 decoder는 제거.
- **결과**: GPT-2 기준 **Coconut +8.2%, CODI +4.3%**. LLaMA 1B/3B/8B에서 일관된 +1.5-9.0%p 향상
- **안정성**: 8-16 implicit token까지 안정 작동 (prior methods는 collapse하는 영역)

**[Latent Reasoning with Supervised Thinking States](https://arxiv.org/abs/2602.08332)** (arXiv 2602.08332, 2026-02)
- **차별점**: input 처리 **도중에** thinking token 생성 (Coconut은 input 처리 후). Teacher-forcing으로 parallelizable
- **결과**: 수학 task에서 CoT와 격차 narrowing, 2-Hop QA에서는 CoT 성능 매칭하면서 latency 개선

**[Latent Thoughts Tuning (LT-Tuning)](https://arxiv.org/abs/2602.10229)** (arXiv 2602.10229, 2026-02)
- **차별점**: **Context-Prediction-Fusion 메커니즘**. Contextual hidden state와 vocabulary embedding space의 predictive semantic guidance를 함께 활용
- **해결한 문제**: hidden state를 반복 input embedding으로 쓸 때 발생하는 distributional mismatch (Coconut의 functional collapse)

**[Inference-Time Rethinking with Latent Thought Vectors](https://arxiv.org/abs/2602.06584)** (arXiv 2602.06584, 2026-02)
- **차별점**: reasoning을 (latent thought vector) + (decoder)로 factorize. Inference에서 **Gibbs-style alternating optimization**으로 latent vector 반복 개선
- **충격적 결과**: **0.2B 모델이 30 rethinking iteration으로 3B 모델을 능가** (GSM8K). 즉 작은 모델 + heavy inference compute가 큰 모델을 이김
- **시사점**: inference-time compute scaling의 새로운 축

이 7편을 정리하면, Coconut의 한계가 빠르게 메워지고 있어요.

| 방법 | 해결한 Coconut 한계 |
|---|---|
| Abstract CoT | 해석성 부재 (#5) |
| RLTT | GSM8K underperformance (#1) — process reward로 보완 |
| PLaT | Fixed c 값 의존 (#2 curriculum essential) — dynamic termination |
| SIM-CoT | Curriculum forgetting (#3) — auxiliary decoder가 step-level signal 제공 |
| Supervised Thinking States | Sequential 의존성 (#4) — parallelizable |
| LT-Tuning | Distributional mismatch (#3) — Context-Prediction-Fusion |
| Inference-Time Rethinking | GPT-2 scale 한정 (#6) — 작은 모델로 inference compute 활용 |

### 9.3 Looped 아키텍처의 부상 — Ouro

**[Ouro: Looped Language Models](https://ouro-llm.github.io/)** (arXiv 2510.25741, ByteDance + UCSC + Princeton + CMU 외)
- **핵심**: parameter-shared **looped transformer**. 같은 block을 4 recurrent step (R4) 반복 적용
- **아키텍처**: standard decoder-only (RoPE, SwiGLU, sandwich norm) + **entropy-regularized objective**로 학습된 depth allocation
- **효율**: **1.4B가 4B 대등, 2.6B가 8B 대등** (parameter efficiency 2-3x)
- **학습 규모**: 7.7T 토큰 (3T standard pre-training + 3T 추가 + 1.4T CoT annealing + 20B long-context + 300B mid-training + SFT)
- **체크포인트**: HuggingFace 공개 (1.4B/2.6B + Thinking variants)

Ouro는 RLTT(9.2)와 함께 보면 "looped architecture가 latent reasoning의 next infrastructure"가 되어가는 흐름을 보여줘요. HRM이 task-specific 27M 모델이었다면, Ouro는 **general-purpose 1.4-2.6B looped LM**으로 scale up.

### 9.4 도메인 확장 — Vision-Language, VLA

**[LaRA-VLA: Latent Reasoning for VLA](https://arxiv.org/abs/2602.01166)** (arXiv 2602.01166, 2026-02, ICML 2026)
- **타겟**: Vision-Language-Action 모델 (로봇 제어)
- **핵심**: multi-modal CoT를 continuous latent representation으로 internalize. 3-stage curriculum (explicit CoT → latent reasoning → action-conditioned latent dynamics)
- **결과**: 명시적 CoT 대비 **90% inference latency 감소**. simulation과 real-robot manipulation 모두에서 SOTA VLA 능가
- **해결한 문제**: "discrete reasoning representation이 continuous perception/control과 mismatch"

**[Visual Latents Know More Than They Say](https://arxiv.org/abs/2605.02735)** (arXiv 2605.02735, 2026-05)
- **타겟**: MLLM (Multimodal LLM)
- **핵심 발견**: visual latent가 학습 중에 semantically enriched되지만, **answer prediction 시점에 systematically suppressed**된다는 optimization pathology 발견. 모델이 직접 visual input shortcut을 쓰고 latent를 우회.
- **해결**: 2-stage inference-time optimization (no parameter update)
  - Stage I: Query-guided contrastive alignment로 visual latent 활성화
  - Stage II: Confidence-progression reward로 latent를 route하도록 유도
- **결과**: 8개 벤치마크 × 4개 모델 backbone에서 일관된 개선

이 두 편이 합해서 보내는 메시지: **latent reasoning은 multimodal·embodied domain으로 확장 중이고, 거기서 더 큰 effective gain이 나옴**. Robot 분야는 90%의 latency를 줄이는 게 production deployment에 직결되니까요.

### 9.5 분야 입장 + 벤치마크 — 메타-레벨 정리

**[LLM Reasoning Is Latent, Not the Chain of Thought](https://arxiv.org/abs/2604.15726)** (arXiv 2604.15726, 2026-04, Wenshuo Wang)
- **포지션**: "CoT가 reasoning을 충실하게 드러낸다"는 통념을 정면 반박
- **방법론**: 3가지 가설 비교 framework 제시
  - **H1**: latent state trajectory가 진짜 reasoning (저자의 default)
  - **H2**: explicit surface CoT가 reasoning
  - **H0**: 성능 향상은 단순한 compute scaling
- **방법**: "compute-audited worked exemplars" + latent intervention + matched budget expansion으로 셋을 분리
- **권고**: latent state dynamics를 LLM 연구의 default 대상으로 삼아야 함

이 논문은 일종의 **position paper**예요. Anthropic의 "Faithfulness of CoT" 시리즈와 함께, "보이는 CoT만 보지 말고 latent를 보라"는 흐름의 한 축을 형성하고 있어요. 8번 섹션의 해석 연구가 이 권고를 기술적으로 뒷받침하는 거고요.

**[LongCoT: Long-Horizon CoT Benchmarking](https://arxiv.org/abs/2604.14140)** (arXiv 2604.14140, 2026-04)
- **구성**: 2,500개 expert-designed 문제, 5개 도메인 (chemistry, mathematics, computer science, chess, logic)
- **정의**: "long-horizon" = tens to hundreds of thousands of reasoning tokens
- **충격적 결과**:
  - **GPT 5.2: 9.8%**
  - **Gemini 3 Pro: 6.1%**
  - 각 개별 step은 SOTA 모델도 풀 수 있는데, 이들을 길게 chain한 전체 추론에서 무너짐
- **의의**: 지금까지의 reasoning 벤치마크(GSM8K, AIME, GPQA)가 측정하지 못한 long-horizon 능력의 결함을 드러냄

LongCoT가 보여주는 건 **현재 reasoning 모델들도 long-horizon에서는 사실상 못한다**는 거예요. Coconut류 latent reasoning이 이 영역에서 어떤 차이를 보일지는 미평가 — 8번 섹션 8.3의 RLTT가 이 방향으로 가는 첫 시도예요.

### 9.6 5개월 흐름 정리

| 카테고리 | 논문 수 | 메시지 |
|---|---|---|
| 이론 | 2 | "왜 어려운가"를 정량적으로 증명 (exploration-execution trade-off, high-order dependency decay) |
| 새 학습 방법 | 7 | Coconut의 한계 7가지가 하나씩 메워지는 중 |
| Looped 아키텍처 | 1 (+RLTT) | HRM의 27M에서 Ouro의 1.4-2.6B general-purpose로 scale up |
| 도메인 확장 | 2 | VLA 90% latency 감소, MLLM 8 벤치마크 SOTA — **production impact가 multimodal에서 더 큼** |
| 입장 + 벤치마크 | 2 | "CoT는 reasoning의 표면일 뿐"이라는 분야 합의 형성 + long-horizon에서 모든 모델 실패 드러남 |

### 9.7 2026 시기를 어떻게 봐야 하나

5개월에 12편이 나왔다는 건 단순한 활황을 넘어요. **분야가 토대(Coconut) → 다양화(2024-2025) → 통합과 성숙(2026)** 단계로 진입했다는 신호예요.

특히 주목해야 할 변화 3가지:

1. **이론과 실증의 결합**: 9.1의 두 이론 논문이 9.2의 학습 방법론들과 곧장 연결돼요. SIM-CoT의 step-level supervision은 Capabilities&Limits 논문의 "curriculum learning essential" 정리에 기반.

2. **Scale-up이 시작됨**: Ouro가 1.4-2.6B로 가면서 "latent reasoning은 GPT-2 scale에서만 작동하는 트릭"이라는 우려가 깨짐. RLTT가 process reward로 본격 RL과 결합.

3. **Production 시그널**: VLA 90% latency 감소, MLLM 8 벤치마크 SOTA — 이건 더 이상 academic curiosity가 아니라 **deployment 차원의 의미가 있는 결과**예요.

10번 섹션에서 이걸 분야 전체 지형도에 어떻게 통합할지 다뤄볼게요.

---

## 10. 5가지 흐름 종합 — 분야 전체 지형도
...
2024-12 │ Coconut           — continuous thought feedback (COLM 2025) ★ 베이스 논문
2025-02 │ CODI              — self-distillation으로 GSM8K 격차 해결 (EMNLP 2025)
2025-05 │ Soft Thinking     — training-free concept token (NeurIPS 2025)
2025-06 │ HRM               — 27M으로 ARC-AGI 격파
2025-07 │ Survey            — 분야 분류 체계 정립
2025-09 │ SIM-CoT, LTO      — auxiliary decoder supervision, latent reward signal (ICLR 2026)
2026-01 │ PLaT, CoT Compression Theory — planning view + 이론적 한계
2026-02 │ Capabilities&Limits, RLTT, Supervised Thinking States, LT-Tuning, Inference-Time Rethinking, Dynamics
2026-02 │ LaRA-VLA          — VLA 도메인 확장 (ICML 2026)
2026-04 │ Abstract CoT, LongCoT, LLM-Reasoning-Is-Latent, RecursiveMAS
2026-05 │ Visual Latents (MLLM unsilencing)
```

2년이 채 안 되는 기간에 single-model latent reasoning이 system-level까지 일반화됐고, 2026년 5개월에만 12편이 나오면서 **분야가 성숙기에 진입**했어요. 이 흐름이 다음으로 어디로 갈지는 11번 섹션에서 다뤄볼게요.
...
| **2026 신규** | — | — |
| 9a | Capabilities and Fundamental Limits of Latent CoT | [arXiv:2602.01148](https://arxiv.org/abs/2602.01148) |
| 9b | Chain Of Thought Compression: A Theoretical Analysis (ALiCoT) | [arXiv:2601.21576](https://arxiv.org/abs/2601.21576) |
| 9c | Thinking Without Words: Abstract Chain-of-Thought | [arXiv:2604.22709](https://arxiv.org/abs/2604.22709) |
| 9d | RLTT: Rewarding Latent Thought Trajectories in Looped LM | [arXiv:2602.10520](https://arxiv.org/abs/2602.10520) |
| 9e | PLaT: Latent CoT as Planning | [arXiv:2601.21358](https://arxiv.org/abs/2601.21358) |
| 9f | SIM-CoT: Supervised Implicit CoT (ICLR 2026) | [arXiv:2509.20317](https://arxiv.org/abs/2509.20317) / [GitHub](https://github.com/InternLM/SIM-CoT) |
| 9g | Latent Reasoning with Supervised Thinking States | [arXiv:2602.08332](https://arxiv.org/abs/2602.08332) |
| 9h | Latent Thoughts Tuning (LT-Tuning) | [arXiv:2602.10229](https://arxiv.org/abs/2602.10229) |
| 9i | Inference-Time Rethinking with Latent Thought Vectors | [arXiv:2602.06584](https://arxiv.org/abs/2602.06584) |
| 9j | Ouro: Looped Language Models | [arXiv:2510.25741](https://arxiv.org/abs/2510.25741) / [ouro-llm.github.io](https://ouro-llm.github.io/) |
| 9k | LaRA-VLA: Latent Reasoning for Vision-Language-Action (ICML 2026) | [arXiv:2602.01166](https://arxiv.org/abs/2602.01166) |
| 9l | Visual Latents Know More Than They Say (MLLM Unsilencing) | [arXiv:2605.02735](https://arxiv.org/abs/2605.02735) |
| 9m | LLM Reasoning Is Latent, Not the Chain of Thought | [arXiv:2604.15726](https://arxiv.org/abs/2604.15726) |
| 9n | LongCoT: Long-Horizon Chain-of-Thought Benchmarking | [arXiv:2604.14140](https://arxiv.org/abs/2604.14140) |
| **기타** | — | — |
...
**Q1. 자연어 토큰 대신 hidden state로 추론하면 좋다는 latent reasoning의 핵심 근거, "대역폭 차이"는 무엇을 말하는 걸까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 토큰 하나가 담는 정보(약 15 bits)에 비해 hidden state 하나(2,560-dim FP16 기준 약 40,960 bits)는 약 2,700배 큰 표현 용량을 갖는다는 점이에요.

**해설**: CoT는 생각을 매 step마다 vocab 크기(~32k)에 묶인 discrete 토큰으로 압축해야 하는데, 이 순간 엄청난 정보 손실이 일어나죠. Latent reasoning은 이 압축 단계를 건너뛰고 hidden state를 그대로 다음 입력으로 쓰니까 정보 손실이 없어요. 이게 분야 전체를 관통하는 가장 기본적인 motivation이에요.

</details>

**Q2. OX 문제예요. "Coconut은 GSM8K 수학 벤치마크에서 discrete CoT보다 높은 정확도를 기록했다." 맞을까요, 틀릴까요?**
...
**해설**: 의외로 Coconut은 GSM8K에서 34.1%로, discrete CoT의 42.9%보다 8.8%p나 낮았어요. 대신 search가 필요한 ProsQA에서는 96.7% vs 77.5%로 압도했고, 토큰 예산도 3배 절감했죠. "latent가 항상 더 좋다"가 아니라 exploration형 task에 강하고 execution형 task에 약하다는 게 정직한 결론이에요.

</details>

**Q3. Coconut과 Soft Thinking은 둘 다 latent reasoning이지만 결정적인 차이가 있어요. 적용 방식 관점에서 두 방법을 구분해 볼까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: Coconut은 multi-stage curriculum으로 모델을 새로 학습해야 하지만, Soft Thinking은 가중치를 전혀 바꾸지 않는 training-free 방법이에요.

**해설**: Coconut은 hidden state를 다음 input embedding으로 직접 넘기는 구조라 정교한 curriculum 학습이 필수인데요(없으면 학습 실패), Soft Thinking은 argmax로 토큰 하나를 고르는 대신 vocabulary 전체의 확률 가중합으로 concept token을 만들어요. 그래서 기존 모델에 오늘 당장 적용할 수 있다는 게 Soft Thinking의 가장 큰 실용적 장점이죠.

</details>

**Q4. HRM이 27M 파라미터로 ARC-AGI에서 거대 모델들을 이긴 비결로, 뇌에서 영감을 받은 두 모듈 구조가 있었죠. 각각 어떤 역할을 할까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: H-module(high-level)은 느리게 추상적인 plan을 만들고, L-module(low-level)은 매 타임스텝마다 빠르게 세부 계산을 처리해요.

**해설**: 뇌의 prefrontal cortex가 느린 timescale로 정보를 통합하고 감각 영역이 빠르게 처리하는 구조를 모방한 거예요. L-module이 fixed point에 수렴하면 H-module이 한 번 업데이트되는 식으로 두 시간 스케일이 맞물려 돌아가죠. 이 구조로 단 1,000개 샘플, pretraining 없이 ARC-AGI 40.3%를 달성했어요.

</details>

**Q5. Coconut에서 명시적으로 학습시킨 적이 없는데 나타난 "BFS-like behavior"란 무엇이고, 왜 이런 게 나올 수 있었을까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: Continuous thought가 여러 candidate reasoning path를 동시에 인코딩해서, 마치 너비 우선 탐색(BFS)처럼 병렬로 후보를 탐색하는 행동이에요. Latent 구간에는 supervised signal이 없어서 모델이 "최종 답에 도움되는 latent"를 스스로 발견하다 보니 emergent하게 나타났죠.

**해설**: CE loss가 자연어 토큰 부분에만 걸리고 latent 구간(`<bot>...<eot>`)에는 ground truth가 없다는 게 핵심이에요. 첫 continuous thought를 강제 디코딩해 보니 여러 후보 path의 시작 토큰들이 비슷한 확률로 분포해 있었고, 후속 연구(Dynamics)가 이걸 mechanistic하게 확증했어요. 단일 토큰을 골라야 하는 CoT의 premature commitment 문제를 latent가 자연스럽게 우회한 셈이죠.

</details>

**Q6. RecursiveMAS가 기존 multi-agent 시스템(LangChain류)과 다른 점은 뭘까요? 그리고 "최종 round만 텍스트"라는 설계가 왜 효율적인지도 같이 떠올려 보세요.**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 기존 MAS는 agent끼리 자연어 텍스트로 통신하는데, RecursiveMAS는 모든 intermediate 통신을 latent space에서 하고 최종 round에서만 텍스트로 디코딩해요. Agent 경계마다 반복되던 vocab projection(15-bit 병목)이 사라지니까 정확도는 +8.3% 오르면서 추론은 1.2-2.4배 빨라지고 토큰은 34.6-75.6% 줄었죠.

**해설**: 핵심 컴포넌트는 RecursiveLink라는 가벼운 MLP인데, base LLM은 전부 frozen하고 약 13M(전체의 0.31%)만 학습해요. Coconut의 "hidden state를 다음 input으로" 아이디어가 같은 agent의 self-loop를 넘어 agent 경계까지 확장된 거예요. 이게 latent reasoning이 single model을 넘어 system-level scaling axis가 될 수 있다는 첫 대규모 실증이죠.

</details>

**Q7. 응용 시나리오 하나 볼게요. 이미 배포된 32B reasoning 모델이 있는데, 재학습 없이 어려운 수학 문제(AIME급) 성능을 끌어올리고 싶어요. 이 블로그에서 소개한 방법 중 뭐가 가장 적합하고, 그 이유는 뭘까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: Soft Thinking이에요. 가중치 변경 없이 inference만 바꾸는 training-free 방법인 데다, QwQ-32B에서 AIME 2024 +6.45%p로 어려운 문제일수록 효과가 컸거든요.

**해설**: Coconut이나 HRM은 새로 학습하거나 아예 새 아키텍처가 필요해서 "이미 배포된 모델"이라는 조건에 안 맞아요. Soft Thinking은 argmax 대신 확률 가중합으로 다음 embedding을 만드는 것뿐이라 즉시 적용 가능하죠. 게다가 generation length도 11.6~22.4% 줄어서 비용 측면에서도 이득이에요.

</details>

**Q8. Latent reasoning의 가장 자주 지적되는 약점이 "해석성 부재"인데요. 2025-2026 해석 연구들이 발견한 것 중 alignment 관점에서 양날의 검이라고 평가된 발견은 뭘까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: LTO 연구가 발견한 "latent thought 안에 reward signal이 인코딩되어 있다"는 사실이에요. 명시적 reward supervision 없이도 latent thought만 보고 정답/오답을 예측할 수 있었죠.

**해설**: 좋은 면은 latent reasoning의 품질을 외부에서 측정하고 test-time에 더 좋은 latent를 선택해 성능을 올릴 수 있다는 것이고요. 나쁜 면은 모델이 "내부에서는 정답을 알면서 외부로는 다른 답을 출력"할 가능성을 시사한다는 거예요. RLHF가 surface 토큰에만 작용하는 한 latent가 alignment training의 사각지대가 될 수 있다는 우려와 직결되죠.

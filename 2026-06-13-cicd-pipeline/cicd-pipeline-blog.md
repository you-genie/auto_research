...
> 📊 **발표자료**: [cicd-pipeline-presentation.pptx](./cicd-pipeline-presentation.pptx)
...
**Q1. 지속적 전달(Continuous Delivery)과 지속적 배포(Continuous Deployment)의 결정적인 차이는 뭘까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 프로덕션 배포가 수동(버튼 누름)이냐 자동이냐의 차이예요. 지속적 전달은 배포 가능한 상태까지만 자동화하고 마지막 배포는 사람이 승인하는 반면, 지속적 배포는 테스트 통과 후 프로덕션까지 사람 개입 없이 자동으로 나가요.

**해설**: 둘 다 약자가 CD라서 헷갈리기 쉬운데요, 자동화 범위의 끝이 어디냐가 핵심이에요. 지속적 배포는 Netflix, Amazon처럼 테스트 커버리지가 아주 촘촘한 성숙한 팀이 하는 고급 단계라서, 대부분의 팀은 CI + 지속적 전달 조합으로 운영하죠.

</details>

**Q2. OX 퀴즈! "GitHub Actions 러너는 GitHub 서버가 러너에게 직접 접속해서 잡(Job)을 밀어넣는(push) 방식으로 동작한다." 맞을까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: X예요. 반대로 러너가 GitHub 서버에 "새 잡 있어요?"라고 계속 물어보는 롱폴링(Long Polling) 방식이에요.

**해설**: 러너가 먼저 GitHub Actions 서비스에 인증하고 HTTPS 롱폴링 연결을 맺으면, 서버는 최대 50초 동안 연결을 열어두고 새 잡이 생기면 메시지를 보내줘요. 러너가 능동적으로 가져가는(pull) 구조라서 러너가 방화벽 안에 있어도 잘 동작하는 거죠.

</details>

**Q3. 회사 내부 네트워크의 DB에 접근하는 통합 테스트를 CI에서 돌려야 해요. GitHub-Hosted 러너와 Self-Hosted 러너 중 어떤 걸 골라야 하고, 이때 주의할 점은 뭘까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: Self-Hosted 러너가 필수예요. GitHub-Hosted 러너는 회사 내부망에 접근할 수 없거든요. 다만 OS 패치 같은 유지관리를 직접 해야 하고, 잡 간 환경 오염(이전 잡의 파일이 남는 문제)과 퍼블릭 저장소에서의 보안 위험(외부인의 PR이 내 서버에서 실행될 수 있음)을 주의해야 해요.

**해설**: GitHub-Hosted는 설정 없이 바로 쓸 수 있고 잡마다 깨끗한 VM이 생겨서 편하지만, 내부망 접근과 특수 하드웨어(GPU 등) 활용은 불가능해요. 그래서 내부 시스템 접근이나 GPU 빌드가 필요하면 Self-Hosted를 선택하는 거예요.

</details>

**Q4. 팀 규모가 커져서 셀프 호스팅 러너 수십 대를 관리하기 버거워졌어요. 쿠버네티스 위에서 러너를 자동으로 늘리고 줄여주는 도구는 뭐고, 어떻게 동작하나요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: Actions Runner Controller(ARC)예요. ARC가 GitHub에 롱폴링 연결을 유지하다가 새 잡이 생기면 쿠버네티스 API로 Runner Pod를 만들고, 잡이 끝나면 Pod를 자동 삭제해요. 잡이 없을 땐 러너를 0개까지 스케일 다운하죠.

**해설**: 잡마다 깨끗한 컨테이너 환경이 새로 생기니까 셀프 호스팅의 고질병인 환경 오염 문제가 사라지고, 안 쓸 때는 0개로 줄어드니 비용 효율도 좋아요. 셀프 호스팅의 장점과 호스팅 러너의 깔끔함을 동시에 잡는 방식이에요.

</details>

**Q5. 블루-그린 배포와 카나리 배포, 각각 어떤 상황에 어울릴까요? 장단점을 기준으로 구분해 보세요.**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 블루-그린은 동일한 환경 두 개를 두고 트래픽을 한 번에 전환하는 방식이라 롤백이 빠르고 간단하지만 인프라 비용이 2배예요. 카나리는 신버전을 10% → 25% → 50%처럼 소수 사용자에게 점진적으로 늘려가는 방식이라 위험이 가장 낮지만, 설정이 복잡하고 촘촘한 모니터링이 필요해요.

**해설**: "문제 생기면 즉시 되돌리기"가 최우선이고 비용을 감당할 수 있다면 블루-그린, 실제 사용자 반응으로 검증하면서 위험을 최소화하고 싶다면 카나리가 어울려요. 둘 다 프로덕션 배포 위험을 줄이는 핵심 전략이죠.

</details>

**Q6. 워크플로에서 타사 액션을 쓸 때 `uses: actions/checkout@v4`처럼 태그로 지정하는 것보다 더 안전한 방법은 뭐고, 왜 그럴까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 커밋 SHA로 버전을 고정하는 거예요(예: `uses: actions/checkout@11bd719...`). 태그는 나중에 다른 커밋을 가리키도록 변경될 수 있지만, 커밋 SHA는 불변(immutable)이라서요.

**해설**: 2025년에 실제로 타사 액션 저장소가 해킹당해 워크플로 코드가 변조된 공급망 공격이 있었는데요, 태그(`@v1`)를 쓰던 저장소들은 자동으로 악성 코드를 실행해버렸어요. SHA로 고정해두면 액션 저장소가 해킹돼도 내가 검증한 코드만 실행돼요.

</details>

**Q7. CI/CD에서 AWS에 배포할 때 Access Key 같은 장기 자격증명을 시크릿에 저장하는 대신 요즘 best practice로 자리잡은 인증 방식은 뭘까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: OIDC(OpenID Connect) 기반 인증이에요. 잡이 실행될 때마다 임시 토큰을 발급받아서 인증하니까, 장기 자격증명 자체를 저장할 필요가 없어져요.

**해설**: 시크릿은 아무리 잘 관리해도 노출 위험이 남는데, OIDC는 저장된 비밀 자체가 없으니 노출 위험을 근본적으로 없애는 접근이에요. AWS, GCP, Azure 모두 지원하고, 2025년을 기점으로 표준으로 굳어졌어요. Sigstore가 키 없이 아티팩트에 서명할 때 쓰는 것도 같은 OIDC 신원 기반이죠.

</details>

**Q8. 매트릭스 빌드(strategy.matrix)에서 `os: [ubuntu-latest, macos-latest]`와 `python-version: ['3.10', '3.11', '3.12']`를 설정하면 어떤 일이 일어나고, 이게 언제 특히 유용할까요?**

<details markdown="1">
<summary>✅ 정답 보기</summary>

**정답**: 2개 OS × 3개 Python 버전 = 총 6개의 잡이 조합별로 생성돼서 병렬로 실행돼요. 여러 환경에서 동작해야 하는 라이브러리를 개발할 때 특히 유용해요.

**해설**: 설정 하나로 모든 조합을 자동으로 테스트하니까 "내 환경에선 됐는데"를 환경 조합 차원에서 미리 잡아낼 수 있어요. `fail-fast: false`를 함께 쓰면 한 조합이 실패해도 나머지 조합은 계속 실행돼서, 어느 환경이 문제인지 한 번에 파악할 수 있죠.
